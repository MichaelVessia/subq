...
/**
 * Generate sparse user data (irregular tracking with gaps)
 */
export function generateSparseUserData(): SparseUserData {
  const now = new Date().toISOString()
  const startDate = new Date()
  startDate.setFullYear(startDate.getFullYear() - 1)
  startDate.setHours(8, 0, 0, 0)
  ...
  const phases: Phase[] = [
    ...
    { startWeek: 27, endWeek: 30, active: true, drug: 'Tirzepatide', dose: '2.5mg', notes: 'Switching to Tirzepatide' },
  ...
  // Generate weights with irregular patterns
  type WeighPeriod = {
    startDay: number
    endDay: number
  ...
  for (let day = 0; day <= 365; day++) {
    ...
    if (shouldWeigh) {
      ...
      if (day === 0) notes = 'Starting weight'
        ...
        const randomNotes = ['After big meal', 'Morning weight', 'Before bed', 'Dehydrated probably', 'Feeling bloated']
        notes = randomNotes[Math.floor(Math.random() * randomNotes.length)]!
      }

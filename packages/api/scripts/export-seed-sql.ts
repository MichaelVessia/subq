 ...
 * Run: SEED_USER_ID=xxx SEED_USER_TYPE=consistent bun run packages/api/scripts/export-seed-sql.ts > seed.sql
...
const USER_TYPE = (process.env.SEED_USER_TYPE || 'consistent') as 'consistent' | 'sparse'

if (USER_ID === 'YOUR_USER_ID_HERE') {
  console.error('Set SEED_USER_ID env var to your production user ID')
  console.error('Optionally set SEED_USER_TYPE=sparse for sparse user data')
  process.exit(1)
}
...
if (USER_TYPE === 'consistent') {
  ...
  // Insert injections (no schedules)
  for (const inj of data.injections) {
    sql.push(
      `INSERT INTO injection_logs (id, datetime, drug, source, dosage, injection_site, notes, schedule_id, user_id, created_at, updated_at)`,
    )
    sql.push(
      `VALUES (${escapeSQL(inj.id)}, ${escapeSQL(inj.datetime)}, ${escapeSQL(inj.drug)}, ${escapeSQL(inj.source)}, ${escapeSQL(inj.dosage)}, ${escapeSQL(inj.injectionSite)}, ${escapeSQL(inj.notes)}, NULL, ${escapeSQL(USER_ID)}, ${escapeSQL(inj.createdAt)}, ${escapeSQL(inj.updatedAt)});`,
    )
  }
  sql.push('')

  ...
  describe('getDashboardStats', () => {
    it.effect('returns null when less than 2 data points', () =>
      Effect.gen(function* () {
        yield* setupTables
        yield* clearTables
        yield* insertWeightLog('w1', new Date('2024-01-15T10:00:00Z'), 185, 'user-123')
    ...
    it.effect('calculates dashboard stats correctly', () =>
      Effect.gen(function* () {
        ...
        yield* insertWeightLog('w2', new Date('2024-01-08T10:00:00Z'), 198, 'user-123')
        yield* insertWeightLog('w3', new Date('2024-01-15T10:00:00Z'), 196, 'user-123')
    ...
    it.effect('filters by date range', () =>
      Effect.gen(function* () {
        ...
        yield* insertWeightLog('w2', new Date('2024-01-08T10:00:00Z'), 198, 'user-123')
        yield* insertWeightLog('w3', new Date('2024-01-15T10:00:00Z'), 196, 'user-123')
        yield* insertWeightLog('w4', new Date('2024-01-22T10:00:00Z'), 194, 'user-123')
    ...
    it.effect('isolates data by user', () =>
      Effect.gen(function* () {
        yield* setupTables
        yield* clearTables
        yield* insertWeightLog('w1', new Date('2024-01-01T10:00:00Z'), 200, 'user-123')
        yield* insertWeightLog('w2', new Date('2024-01-08T10:00:00Z'), 198, 'user-123')
        yield* insertWeightLog('w3', new Date('2024-01-01T10:00:00Z'), 150, 'user-456')
        ...
        expect(result).not.toBeNull()
        expect(result!.startWeight).toBe(200)
        expect(result!.endWeight).toBe(198)
        expect(result!.dataPointCount).toBe(2)
      }).pipe(Effect.provide(TestLayer)),
    )
  })

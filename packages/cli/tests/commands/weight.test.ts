/**
 * Tests for the quick-weight command (weight.ts).
 * Tests that the command writes to local weight_logs + outbox and requires login.
 */
import {
  LocalConfig,
  LocalDb,
...
describe('quick-weight command', () => {
  ...
  describe('writes to local database', () => {
    it.effect('calls writeWithOutbox with weight_logs table', () =>
      Effect.gen(function* () {
        ...
        if (Option.isSome(writeOptions)) {
          expect(writeOptions.value.table).toBe('weight_logs')
          expect(writeOptions.value.operation).toBe('insert')
        }
      }),
    )

    it.effect('creates outbox entry for syncing with correct weight value', () =>
      Effect.gen(function* () {
        const lastWriteOptions = yield* Ref.make<Option.Option<WriteWithOutboxOptions>>(Option.none())
        ...
        // Simulate a write with weight and notes
        yield* Effect.gen(function* () {
          const local = yield* LocalDb
          yield* local.writeWithOutbox({
            table: 'weight_logs',
        ...
        if (Option.isSome(writeOptions)) {
          ...
          expect(payload.weight).toBe(183.2)
          expect(payload.notes).toBe('morning weigh-in')
        }
      }),
    )
  })
  ...
  describe('command execution flow', () => {
    ...
    it.effect('writes weight when logged in', () =>
      Effect.gen(function* () {
        // Simulate the full flow with logged in user
        const storedToken = yield* Ref.make<Option.Option<string>>(Option.some('test-token'))
        const writeWithOutboxCalled = yield* Ref.make(false)

...
export class ProductAssistantError extends Schema.TaggedErrorClass<ProductAssistantError>()('ProductAssistantError', {
      ...
      const answer = Effect.fn('ProductAssistant.answer')(
        function* (question: string) {
          ...
          for (const result of response.toolResults) {
            yield* Effect.log(`Tool result: ${result.name} id=${result.id} isFailure=${result.isFailure}`)
          }

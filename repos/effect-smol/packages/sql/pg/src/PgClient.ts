...
export const fromClient = Effect.fnUntraced(function* (options: {
  ...
  let streamClient = options.acquireForStream
    ...
    : Effect.acquireRelease(Effect.as(semaphore.take(1), client), () => semaphore.release(1))
  ...
  return Object.assign(
    ...
    }),
    {
      ...
      notify: (channel: string, payload: string) =>
        Effect.asVoid(
          Effect.scoped(
            Effect.flatMap(options.acquirer, (conn) => conn.executeRaw(`SELECT pg_notify($1, $2)`, [channel, payload])),
          ),
        ),
    },
  )
        ...
        Effect.fnUntraced(function* (_, scope) {
          ...
          return Effect.callback<Arr.NonEmptyReadonlyArray<any>, SqlError | Cause.Done>((resume) => {
            cursor.read(128, (err, rows) => {
              if (err) {
                resume(
                  Effect.fail(new SqlError({ reason: classifyError(err, 'Failed to execute statement', 'stream') })),
                )
...
export const layerFrom = <E, R>(
  ...
  Layer.effectContext(
    Effect.map(acquire, (client) => Context.make(PgClient, client).pipe(Context.add(Client.SqlClient, client))),

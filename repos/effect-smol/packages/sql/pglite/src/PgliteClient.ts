        ...
        : yield* Effect.acquireRelease(
            Effect.tryPromise({
              try: async () => {
                ...
                new SqlError({ reason: classifyError(cause, 'PgliteClient: Failed to connect', 'connect') }),
            }),
...
export const layerFrom = <E, R>(
  ...
  Layer.effectContext(
    Effect.map(acquire, (client) => Context.make(PgliteClient, client).pipe(Context.add(Client.SqlClient, client))),
...
export const layerConfig: (
  ...
  layerFrom(Effect.flatMap(Config.unwrap(config), (resolved) => make(resolved as PgliteClientConfig)))

  ...
  layer(ClientLayer, { timeout: '30 seconds' })((it) => {
    ...
    it.effect('nested transaction commits both', () =>
      Effect.gen(function* () {
        ...
        const rows = yield* sql.unsafe<{ total: number }>(`SELECT count(*)::int AS total FROM tx_nested_commit`)
    ...
    it.effect('nested transaction rollback via savepoint', () =>
      Effect.gen(function* () {
        ...
        const rows = yield* sql.unsafe<{ total: number }>(`SELECT count(*)::int AS total FROM tx_nested_rollback`)

...
const mssqlUniqueViolationConstraintFromMessage = (number: 2601 | 2627, message: unknown): string => {
  ...
  const match =
    number === 2627 ? /\bconstraint\s+'([^']*)'/i.exec(message) : /\bunique index\s+'([^']*)'/i.exec(message)
...
export const layerConfig: (
  ...
  Layer.effectContext(
    Config.unwrap(config).pipe(
      Effect.flatMap(make),
      Effect.map((client) => Context.make(MssqlClient, client).pipe(Context.add(Client.SqlClient, client))),
    ),
...
export const layer = (config: MssqlClientConfig): Layer.Layer<Client.SqlClient | MssqlClient, never | SqlError> =>
  Layer.effectContext(
    Effect.map(make(config), (client) => Context.make(MssqlClient, client).pipe(Context.add(Client.SqlClient, client))),
  ).pipe(Layer.provide(Reactivity.layer))

...
export const layerConfig = (
  ...
  Layer.effectContext(
    Config.unwrap(config).pipe(
      Effect.flatMap(make),
      Effect.map((client) => Context.make(D1Client, client).pipe(Context.add(Client.SqlClient, client))),
    ),
...
export const layer = (config: D1ClientConfig): Layer.Layer<D1Client | Client.SqlClient, Config.ConfigError> =>
  Layer.effectContext(
    Effect.map(make(config), (client) => Context.make(D1Client, client).pipe(Context.add(Client.SqlClient, client))),
  ).pipe(Layer.provide(Reactivity.layer))

...
KeyValueStoreTest.suite('sql-sqlite', KeyValueStore.layerSql().pipe(Layer.provide(ClientLayer)))

KeyValueStoreTest.suite(
  'sql-sqlite-custom-table',
  KeyValueStore.layerSql({ table: 'effect_key_value_store_custom' }).pipe(Layer.provide(ClientLayer)),
)

...
describe('SqlEventLogServer', () => {
  ...
  it.effect('rejects session-auth rebinding for an existing publicKey', () =>
    Effect.gen(function* () {
      ...
      const rpcClient = yield* RpcTest.makeClient(EventLogMessage.EventLogRemoteRpcs).pipe(
        Effect.provide(
          EventLogServer.layerRpcHandlers.pipe(Layer.provide(Layer.succeed(EventLogServer.Storage, storage))),
        ),
      )
  ...
  it.effect('isolates same publicKey across storeIds', () =>
    Effect.gen(function* () {
      ...
      const storeAEntries = yield* storage.changes('client-1', storeIdA, 0).pipe(Stream.take(1), Stream.runCollect)
      const storeBEntries = yield* storage.changes('client-1', storeIdB, 0).pipe(Stream.take(1), Stream.runCollect)
  ...
  it.effect('isolates same storeId across publicKeys', () =>
    Effect.gen(function* () {
      ...
      const clientOneEntries = yield* storage.changes('client-1', storeIdA, 0).pipe(Stream.take(1), Stream.runCollect)
      const clientTwoEntries = yield* storage.changes('client-2', storeIdA, 0).pipe(Stream.take(1), Stream.runCollect)
  ...
  it.effect('keeps deduplication isolated per encrypted scope', () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        (yield* storage.changes('client-1', storeIdA, 0).pipe(Stream.take(1), Stream.runCollect)).map(
          (entry) => entry.sequence,
        ),
        [1],
      )
      assert.deepStrictEqual(
        (yield* storage.changes('client-1', storeIdB, 0).pipe(Stream.take(1), Stream.runCollect)).map(
          (entry) => entry.sequence,
        ),
        [1],
      )
      assert.deepStrictEqual(
        (yield* storage.changes('client-2', storeIdA, 0).pipe(Stream.take(1), Stream.runCollect)).map(
          (entry) => entry.sequence,
        ),
        [1],
      )

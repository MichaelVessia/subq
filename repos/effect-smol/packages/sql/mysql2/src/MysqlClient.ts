...
export const layerConfig = (
  ...
  Layer.effectContext(
    Config.unwrap(config).pipe(
      Effect.flatMap(make),
      Effect.map((client) => Context.make(MysqlClient, client).pipe(Context.add(Client.SqlClient, client))),
    ),
...
export const layer = (
  ...
  Layer.effectContext(
    Effect.map(make(config), (client) => Context.make(MysqlClient, client).pipe(Context.add(Client.SqlClient, client))),
  ).pipe(Layer.provide(Reactivity.layer))

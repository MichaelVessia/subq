          ...
          return Effect.callback<Clickhouse.InsertResult, SqlError>((resume) => {
            ...
            client
              ...
              .then(
                ...
                (cause) =>
                  resume(
                    Effect.fail(new SqlError({ reason: classifyError(cause, 'Failed to insert data', 'insert') })),
                  ),
              )
            return Effect.suspend(() => {
              controller.abort()
 ...
 * @since 1.0.0
 */
export const QueryId = Context.Reference<string | undefined>('@effect/sql-clickhouse/ClickhouseClient/QueryId', {
 ...
 * @since 1.0.0
 */
export const ClickhouseSettings: Context.Reference<NonNullable<Clickhouse.BaseQueryParams['clickhouse_settings']>> =
...
export const layerConfig: (
  ...
  Layer.effectContext(
    Config.unwrap(config).pipe(
      Effect.flatMap(make),
      Effect.map((client) => Context.make(ClickhouseClient, client).pipe(Context.add(Client.SqlClient, client))),
    ),
...
export const layer = (
  ...
  Layer.effectContext(
    Effect.map(make(config), (client) =>
      Context.make(ClickhouseClient, client).pipe(Context.add(Client.SqlClient, client)),
    ),
  ).pipe(Layer.provide(Reactivity.layer))

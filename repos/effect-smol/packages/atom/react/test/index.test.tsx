...
describe('atom-react', () => {
  ...
  describe('hydration', () => {
    test('basic hydration with number atom and result atoms', () => {
      ...
      ;(serverRegistry.getNodes().get('success') as any).setValue(AsyncResult.success(123))
      serverRegistry.mount(atomResult2)
      ;(serverRegistry.getNodes().get('errored') as any).setValue(AsyncResult.failure(Cause.fail('error')))
    ...
    test('HydrationBoundary splits new vs existing atoms', () => {
      const newAtom = Atom.make(0).pipe(Atom.serializable({ key: 'new-atom', schema: Schema.Number }))
    ...
    test('dehydrate + JSON roundtrip + hydrate works (SSR simulation)', () => {
      ...
      ;(serverRegistry.getNodes().get('ssr-wire') as any).setValue(AsyncResult.success(42))
      const dehydratedState = Hydration.dehydrate(serverRegistry)
      ...
      function TestComponent() {
        const value = useAtomValue(atom)
        return <div data-testid="ssr-wire-value">{AsyncResult.isSuccess(value) ? value.value : 'not-success'}</div>
      }

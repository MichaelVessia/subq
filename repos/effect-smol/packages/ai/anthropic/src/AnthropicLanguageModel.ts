              ...
              case 'tool-call': {
                if (part.providerExecuted) {
                  ...
                  if (isMcpTool) {
                    ...
                    (part.params.type === 'bash_code_execution' || part.params.type === 'text_editor_code_execution')
                  ) {
                    content.push({
                  ...
                  const caller = Predicate.isNotNullish(options?.caller)
                    ? options.caller.type === 'code_execution_20250825' && Predicate.isNotNullish(options.caller.toolId)
                      ? ({
      ...
      case 'tool_use': {
        ...
        if (options.responseFormat.type === 'json') {
          ...
          parts.push({
            ...
            name: part.name,
            params,
            ...(Predicate.isNotUndefined(callerInfo) ? { metadata: { anthropic: { caller: callerInfo } } } : undefined),
          })
        }
      ...
      case 'mcp_tool_result': {
        ...
        if (Predicate.isNotUndefined(toolCall)) {
          parts.push({
            ...
            metadata: {
              anthropic: {
                ...(Predicate.isNotNullish(mcpMetadata) ? { mcp_tool: mcpMetadata } : undefined),
              },
            },
          })
        }
...
const annotateRequest = (span: Span, request: typeof Generated.BetaCreateMessageParams.Encoded): void => {
 ...
 */
const isUrlData = (data: (typeof Prompt.FilePart.Type)['data']): data is URL => data instanceof URL || isUrlString(data)
  ...
  if (citation.type === 'page_location' || citation.type === 'char_location') {
    ...
    if (Predicate.isNotUndefined(citedDocument)) {
      ...
      return {
        ...
        ...(Predicate.isNotUndefined(citedDocument.fileName) ? { fileName: citedDocument.fileName } : undefined),
        metadata: { anthropic: metadata },
      }
    }
  }

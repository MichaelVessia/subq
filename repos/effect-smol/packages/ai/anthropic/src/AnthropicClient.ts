...
export interface Service {
  ...
  ) => Stream.Stream<Type, HttpClientError.HttpClientError | Schema.SchemaError | Sse.Retry, DecodingServices>
...
export const make = Effect.fnUntraced(
  function* (options: Options): Effect.fn.Return<Service, never, HttpClient.HttpClient> {
      ...
      HttpClient.mapRequest((request) =>
        request.pipe(
          ...
          Predicate.isNotUndefined(options.apiKey)
            ? HttpClientRequest.setHeader(RedactedAnthropicHeaders.AnthropicApiKey, Redacted.value(options.apiKey))
            : identity,
    ...
    const createMessageStream: Service['createMessageStream'] = (options) => {
      ...
      return httpClientOk.execute(request).pipe(
        ...
        Effect.catchTag('HttpClientError', (error) => Errors.mapHttpClientError(error, 'createMessageStream')),
      )
    }
  ...
  Effect.updateService(Headers.CurrentRedactedNames, Array.appendAll(Object.values(RedactedAnthropicHeaders))),
)

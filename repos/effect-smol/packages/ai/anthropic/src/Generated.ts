...
export const BetaAllThinkingTurns = Schema.Struct({ type: Schema.Literal('all').annotate({ title: 'Type' }) }).annotate(
...
export const BetaDirectCaller = Schema.Struct({ type: Schema.Literal('direct').annotate({ title: 'Type' }) }).annotate({
  title: 'DirectCaller',
...
export const BetaFileMetadataSchema = Schema.Struct({
  ...
  mime_type: Schema.String.annotate({ title: 'Mime Type', description: 'MIME type of the file.' })
    .check(Schema.isMinLength(1))
    .check(Schema.isMaxLength(255)),
  size_bytes: Schema.Number.annotate({ title: 'Size Bytes', description: 'Size of the file in bytes.' })
    .check(Schema.isInt())
    .check(Schema.isGreaterThanOrEqualTo(0)),
...
export const BetaServerToolCaller = Schema.Struct({
  tool_id: Schema.String.annotate({ title: 'Tool Id' }).check(Schema.isPattern(new RegExp('^srvtoolu_[a-zA-Z0-9_]+$'))),
...
export const BetaSkill = Schema.Struct({
  skill_id: Schema.String.annotate({ title: 'Skill Id', description: 'Skill ID' })
    .check(Schema.isMinLength(1))
...
export const BetaToolChoiceNone = Schema.Struct({ type: Schema.Literal('none').annotate({ title: 'Type' }) }).annotate({
...
export const BetaRequestMCPToolResultBlock = Schema.Struct({
  ...
  tool_use_id: Schema.String.annotate({ title: 'Tool Use Id' }).check(Schema.isPattern(new RegExp('^[a-zA-Z0-9_-]+$'))),
...
export const BetaRequestTextEditorCodeExecutionToolResultError = Schema.Struct({
  ...
  error_message: Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ title: 'Error Message' })),
...
export const BetaClearThinking20251015 = Schema.Struct({
  keep: Schema.optionalKey(
    Schema.Union([
      Schema.Union([BetaThinkingTurns, BetaAllThinkingTurns], { mode: 'oneOf' }),
      Schema.Literal('all'),
    ]).annotate({
      title: 'Keep',
      description:
...
export const BetaContextManagementConfig = Schema.Struct({
  edits: Schema.optionalKey(
    Schema.Array(
      ...
      .annotate({ title: 'Edits', description: 'List of context management edits to apply' })
...
export const BetaInputContentBlock = Schema.Union(
  [
    ...
    Schema.Struct({
      ...
      caller: Schema.optionalKey(
        Schema.Union([BetaDirectCaller, BetaServerToolCaller, BetaServerToolCaller_20260120], {
          mode: 'oneOf',
  ...
  const decodeError =
    ...
    (response: HttpClientResponse.HttpClientResponse) =>
      Effect.flatMap(HttpClientResponse.schemaBodyJson(schema)(response), (cause) =>
        Effect.fail(AnthropicClientError(tag, cause, response)),
      )
  return {
    httpClient,
...
export interface AnthropicClient {
   ...
   * Learn more about token counting in our [user guide](https://docs.claude.com/en/docs/build-with-claude/token-counting)
   */
  readonly betaMessagesCountTokensPost: <Config extends OperationConfig>(options: {
   ...
   * Upload File
   */
  readonly betaUploadFileV1FilesPost: <Config extends OperationConfig>(options: {
   ...
   * Create Skill
   */
  readonly betaCreateSkillV1SkillsPost: <Config extends OperationConfig>(options: {

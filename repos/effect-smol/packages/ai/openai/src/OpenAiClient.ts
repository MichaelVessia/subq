 ...
 * @category service
 */
export class OpenAiClient extends Context.Service<OpenAiClient, Service>()('@effect/ai-openai/OpenAiClient') {}
...
export const make = Effect.fnUntraced(
  function* (options: Options): Effect.fn.Return<Service, never, HttpClient.HttpClient> {
    ...
    const resolveHttpClient = Effect.map(OpenAiConfig.getOrUndefined, (config) =>
      Predicate.isNotUndefined(config?.transformClient) ? config.transformClient(httpClient) : httpClient,
    )
    ...
    const buildResponseStream = (
      ...
      const stream = response.stream.pipe(
        ...
        Stream.takeUntil(
          (event) => event.data.type === 'response.completed' || event.data.type === 'response.incomplete',
        ),
    ...
    createResponseStream(options) {
      const stream = Stream.unwrap(
        Effect.gen(function* () {
          ...
          yield* send(options).pipe(Effect.forkScoped({ startImmediately: true }))
...
> =>
  Effect.scopedWith((scope) =>
    Effect.flatMap(Scope.provide(makeSocket, scope), (services) => Effect.provideContext(effect, services)),
  )

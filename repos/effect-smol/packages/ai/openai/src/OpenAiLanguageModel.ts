                ...
                if (config.store === true) {
                  ...
                  if (Predicate.isUndefined(message)) {
                    reasoningMessages[id] = {
                      ...
                      ...(Predicate.isNotNull(encryptedContent) ? { encrypted_content: encryptedContent } : undefined),
                    }
...
const isKnownResponseStreamEvent = (event: ResponseStreamEvent): event is KnownResponseStreamEvent =>
...
const makeResponse = Effect.fnUntraced(function* <Tools extends ReadonlyArray<Tool.Any>>({
  ...
  const webSearchTool = options.tools.find(
    (tool) =>
      Tool.isProviderDefined(tool) && (tool.name === 'OpenAiWebSearch' || tool.name === 'OpenAiWebSearchPreview'),
      ...
      case 'message': {
        for (const contentPart of part.content) {
          switch (contentPart.type) {
            case 'output_text': {
              const annotations =
                contentPart.annotations.length > 0 ? { annotations: contentPart.annotations as any } : undefined
...
> {
  ...
  const getOrCreateReasoningPart = (itemId: string, encryptedContent?: string | null): ReasoningPart => {
          ...
          case 'response.output_item.done': {
            switch (event.item.type) {
              ...
              case 'file_search_call': {
                delete activeToolCalls[event.output_index]
                const toolName = toolNameMapper.getCustomName('file_search')
                const results = Predicate.isNotNullish(event.item.results) ? { results: event.item.results } : undefined
                parts.push({
                  type: 'tool-result',
                  id: event.item.id,
                  name: toolName,
              ...
              case 'message': {
                const annotations =
                  activeAnnotations.length > 0 ? { annotations: activeAnnotations.slice() } : undefined
                parts.push({
                  type: 'text-end',
                  id: event.item.id,
...
const annotateRequest = (span: Span, request: typeof OpenAiSchema.CreateResponse.Encoded): void => {
...
const transformToolCallParams = Effect.fnUntraced(function* <Tools extends ReadonlyArray<Tool.Any>>(
  ...
  return yield* (transform(toolParams) as Effect.Effect<unknown, Schema.SchemaError>).pipe(
    Effect.mapError((error) =>
      AiError.make({
        module: 'OpenAiLanguageModel',
        method: 'makeResponse',

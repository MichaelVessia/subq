...
describe('OpenAiClient', () => {
      ...
      Effect.gen(function* () {
        ...
        const client = yield* OpenAiClient.make({
          ...
          transformClient: (client) => {
            ...
            return client.pipe(HttpClient.mapRequest(HttpClientRequest.setHeader('x-openai-transform', 'options')))
          },
        }).pipe(Effect.provide(Layer.succeed(HttpClient.HttpClient, mockClient)))
        ...
        yield* client
          ...
          .pipe(
            OpenAiConfig.withClientTransform((client) => {
              ...
              return client.pipe(HttpClient.mapRequest(HttpClientRequest.setHeader('x-openai-transform', 'config')))
            }),
          )
  ...
  describe('layer', () => {
    ...
    it.effect('layerConfig loads from Config', () => {
      ...
      }).pipe(Layer.provide(HttpClientLayer), Layer.provide(ConfigProvider.layer(configProvider)))

...
export const TopProviderInfo = Schema.Struct({
  ...
  max_completion_tokens: Schema.optionalKey(
    Schema.Number.annotate({ description: 'Maximum completion tokens from the top provider' }).check(Schema.isFinite()),
  ),
...
export const PerRequestLimits = Schema.Struct({
  prompt_tokens: Schema.Number.annotate({ description: 'Maximum prompt tokens per request' }).check(Schema.isFinite()),
...
export const ProviderPreferences = Schema.Struct({
  ...
  max_price: Schema.optionalKey(
    Schema.Struct({
      ...
      completion: Schema.optionalKey(Schema.String.annotate({ description: 'Price per million completion tokens' })),
...
export const Model = Schema.Struct({
  ...
  context_length: Schema.Number.annotate({ description: 'Maximum context length in tokens' }).check(Schema.isFinite()),
...
export type ChatStreamingMessageChunk = {
  ...
  readonly images?: ReadonlyArray<{ readonly type: 'image_url'; readonly image_url: { readonly url: string } }> | null
...
export type OpenAIResponsesInput =
  ...
  | ReadonlyArray<
      ...
      | {
          ...
          readonly role: 'user' | 'system' | 'developer'
          readonly content: ReadonlyArray<
            ResponseInputText | ResponseInputImage | ResponseInputFile | ResponseInputAudio
          >
        }
      | {
          readonly type: 'function_call_output'
...
export const ChatStreamingResponseChunk = Schema.Struct({
  data: Schema.Struct({
    ...
    error: Schema.optionalKey(Schema.Struct({ message: Schema.String, code: Schema.Number.check(Schema.isFinite()) })),
...
export const Message = Schema.Union(
  [SystemMessage, UserMessage, DeveloperMessage, AssistantMessage, ToolResponseMessage],
  { mode: 'oneOf' },
)
...
export const GetGeneration200 = Schema.Struct({
  data: Schema.Struct({
    ...
    total_cost: Schema.Number.annotate({ description: 'Total cost of the generation in USD' }).check(Schema.isFinite()),
    cache_discount: Schema.Number.annotate({ description: 'Discount applied due to caching' }).check(Schema.isFinite()),
    ...
    tokens_prompt: Schema.Number.annotate({ description: 'Number of tokens in the prompt' }).check(Schema.isFinite()),
    tokens_completion: Schema.Number.annotate({ description: 'Number of tokens in the completion' }).check(
      Schema.isFinite(),
    ),
    native_tokens_prompt: Schema.Number.annotate({ description: 'Native prompt tokens as reported by provider' }).check(
      Schema.isFinite(),
    ),
    ...
    native_tokens_cached: Schema.Number.annotate({ description: 'Native cached tokens as reported by provider' }).check(
      Schema.isFinite(),
    ),
...
export const ListProviders200 = Schema.Struct({
  data: Schema.Array(
    Schema.Struct({
      ...
      status_page_url: Schema.optionalKey(Schema.String.annotate({ description: "URL to the provider's status page" })),
    }),
  ),
})
...
export const List200 = Schema.Struct({
  data: Schema.Array(
    Schema.Struct({
      ...
      limit: Schema.Number.annotate({ description: 'Spending limit for the API key in USD' }).check(Schema.isFinite()),
...
export const ListGuardrailsParams = Schema.Struct({
  ...
  limit: Schema.optionalKey(Schema.String.annotate({ description: 'Maximum number of records to return (max 100)' })),
})
...
export const CreateGuardrailRequestJson = Schema.Struct({
  ...
  enforce_zdr: Schema.optionalKey(Schema.Boolean.annotate({ description: 'Whether to enforce zero data retention' })),
})
...
export const ListMemberAssignments200 = Schema.Struct({
  ...
  total_count: Schema.Number.annotate({ description: 'Total number of member assignments' }).check(Schema.isFinite()),
})
  ...
  const decodeError =
    ...
    (response: HttpClientResponse.HttpClientResponse) =>
      Effect.flatMap(HttpClientResponse.schemaBodyJson(schema)(response), (cause) =>
        Effect.fail(OpenRouterClientError(tag, cause, response)),
      )
  return {
    httpClient,
...
export interface OpenRouterClient {
   ...
   */
  readonly exchangeAuthCodeForAPIKey: <Config extends OperationConfig>(options: {
   ...
   */
  readonly sendChatCompletionRequest: <Config extends OperationConfig>(options: {

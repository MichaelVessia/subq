 ...
 * @category service
 */
export class OpenAiClient extends Context.Service<OpenAiClient, Service>()('@effect/ai-openai-compat/OpenAiClient') {}
...
export const make = Effect.fnUntraced(
  function* (options: Options): Effect.fn.Return<Service, never, HttpClient.HttpClient> {
    ...
    const httpClient = baseClient.pipe(
      HttpClient.mapRequest((request) =>
        request.pipe(
          ...
          options.apiKey !== undefined ? HttpClientRequest.bearerToken(Redacted.value(options.apiKey)) : identity,
          ...
          options.projectId !== undefined
            ? HttpClientRequest.setHeader(RedactedOpenAiHeaders.OpenAiProject, Redacted.value(options.projectId))
            : identity,
    ...
    const resolveHttpClient = Effect.map(OpenAiConfig.getOrUndefined, (config) =>
      config?.transformClient !== undefined ? config.transformClient(httpClient) : httpClient,
    )
      ...
      Effect.flatMap(resolveHttpClient, (client) =>
        pipe(
          ...
          Effect.flatMap((response) =>
            Effect.map(decodeResponse(response), (body): [CreateResponse200, HttpClientResponse.HttpClientResponse] => [
              body,
              response,
            ]),
          ),
    ...
    ): [HttpClientResponse.HttpClientResponse, Stream.Stream<CreateResponse200Sse, AiError.AiError>] => {
    ...
    const createResponseStream: Service['createResponseStream'] = (payload) =>
      Effect.flatMap(resolveHttpClient, (client) =>
        pipe(
          ...
          Effect.catchTag('HttpClientError', (error) => Errors.mapHttpClientError(error, 'createResponseStream')),
        ),
      )

    const decodeEmbedding = HttpClientResponse.schemaBodyJson(CreateEmbeddingResponseSchema)

    const createEmbedding = (payload: CreateEmbeddingRequestJson): Effect.Effect<CreateEmbedding200, AiError.AiError> =>
      Effect.flatMap(resolveHttpClient, (client) =>
        pipe(
  ...
  Effect.updateService(Headers.CurrentRedactedNames, Array.appendAll(Object.values(RedactedOpenAiHeaders))),
)
...
export const layerConfig = (options?: {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const apiUrl = options?.apiUrl !== undefined ? yield* options.apiUrl : undefined
      const organizationId = options?.organizationId !== undefined ? yield* options.organizationId : undefined
      const projectId = options?.projectId !== undefined ? yield* options.projectId : undefined
      return yield* make({
        apiKey,
        apiUrl,
        organizationId,
...
const decodeChatCompletionSseData = (data: string): ChatCompletionStreamEvent | undefined => {
  if (data === '[DONE]') {
    return data
  }

...
export class Config extends Context.Service<
  Config,
  Simplify<
    Partial<Omit<CreateResponse, 'input' | 'tools' | 'tool_choice' | 'stream' | 'text'>> & {
      /**
...
const getSystemMessageMode = (model: string): 'system' | 'developer' =>
  model.startsWith('o') || model.startsWith('gpt-5') || model.startsWith('codex-') || model.startsWith('computer-use')
    ? 'developer'
    : 'system'
...
const annotateRequest = (span: Span, request: CreateResponseRequestJson): void => {
...
const toChatCompletionsRequest = (payload: CreateResponse): CreateResponseRequestJson => {
  ...
  const tools =
    payload.tools !== undefined
      ? payload.tools
          .map(toChatTool)
          .filter((tool): tool is NonNullable<ReturnType<typeof toChatTool>> => tool !== undefined)
      : []
  ...
  return {
    ...
    ...(payload.parallel_tool_calls !== undefined ? { parallel_tool_calls: payload.parallel_tool_calls } : undefined),
...
const toChatToolChoice = (toolChoice: OpenAiToolChoice): CreateResponseRequestJson['tool_choice'] | undefined => {
  ...
  const functionName =
    Predicate.hasProperty(toolChoice, 'name') && typeof toolChoice.name === 'string' ? toolChoice.name : toolChoice.type
...
const toChatTool = (tool: OpenAiClientTool): NonNullable<CreateResponseRequestJson['tools']>[number] | undefined => {
...
const toChatMessages = (input: CreateResponse['input']): Array<CreateResponseRequestJson['messages'][number]> => {
...
const toChatMessagesFromItem = (item: InputItem): Array<CreateResponseRequestJson['messages'][number]> => {
...
const stringifyJson = (value: unknown): string => (typeof value === 'string' ? value : JSON.stringify(value))
...
const getItemId = (
  part: Prompt.TextPart | Prompt.ReasoningPart | Prompt.ToolCallPart | Prompt.ToolResultPart,
): string | null => part.options.openai?.itemId ?? null
const getStatus = (part: Prompt.TextPart | Prompt.ToolCallPart | Prompt.ToolResultPart): MessageStatus | null =>
  part.options.openai?.status ?? null
const getEncryptedContent = (part: Prompt.ReasoningPart): string | null => part.options.openai?.encryptedContent ?? null
...
const getUsageDetailNumber = (details: unknown, field: string): number | undefined => {

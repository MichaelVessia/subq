  ...
  describe('generateText', () => {
    ...
    it.effect('preserves multimodal user content order in chat payload', () =>
      Effect.gen(function* () {
        ...
        }).pipe(Effect.provide(OpenAiLanguageModel.model('gpt-4o-mini')), Effect.provide(layer))
    ...
    it.effect('converts dynamic tools to function type', () =>
      Effect.gen(function* () {
        ...
        const functionTool = requestBody.tools?.find(
          (tool: any) => tool.type === 'function' && tool.function?.name === 'DynamicTool',
        )
        assert.isDefined(functionTool)
    ...
    it.effect('maps local shell stream tool calls to local_shell call outputs', () =>
      Effect.gen(function* () {
        ...
        const localShellCall = followUpBody.messages.find(
          (item: any) => item.role === 'assistant' && item.tool_calls?.[0]?.function?.name === 'local_shell',
        )

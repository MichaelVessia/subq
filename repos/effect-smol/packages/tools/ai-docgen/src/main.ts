...
const output = Flag.path('output').pipe(Flag.withAlias('o'), Flag.withDescription('Output file path'))
  ...
  Command.withHandler(
    Effect.fnUntraced(function* ({ directory, output, watch }) {
      ...
      yield* fs.watch(directory).pipe(
        ...
        Stream.switchMap(() => directoryToMarkdown(directory).pipe(Stream.fromEffect)),
        Stream.runForEach(
...
const directoryToMarkdown = Effect.fn('directoryToMarkdown')(function* (directory): Effect.fn.Return<
  string,

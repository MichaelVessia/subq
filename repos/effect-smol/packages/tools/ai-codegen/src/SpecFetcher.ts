 ...
 * @category tags
 */
export const SpecFetcher: Context.Service<SpecFetcher, SpecFetcher> = Context.Service('@effect/ai-codegen/SpecFetcher')
...
export const layer: Layer.Layer<SpecFetcher, never, FileSystem.FileSystem | HttpClient.HttpClient> = Effect.gen(
  function* () {
    ...
    const fetchViaStainlessStats = Effect.fn('fetchViaStainlessStats')(function* (statsUrl: string, provider: string) {
      // Fetch stats.yml

...
export class PostProcessError extends Data.TaggedError('PostProcessError')<{
  ...
  override get message(): string {
    const lines: Array<string> = [`${this.step} failed for ${this.filePath}`, `command: ${this.command}`]
    ...
    const runCommand = Effect.fn('runCommand')(function* (
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const [stdout, stderr] = yield* Effect.all([collectStream(handle.stdout), collectStream(handle.stderr)])

          const exitCode = yield* handle.exitCode.pipe(
            Effect.mapError(
              (cause) => new PostProcessError({ step, command: fullCommand, filePath, stdout, stderr, cause }),
            ),
          )

...
const skipLintFlag = Flag.boolean('skip-lint').pipe(Flag.withDescription('Skip Oxlint step'))

const skipFormatFlag = Flag.boolean('skip-format').pipe(Flag.withDescription('Skip Dprint step'))
...
const generateProvider = Effect.fn('generateProvider')(function* (
  ...
  yield* Console.log(`  ${colors.green}✓${colors.reset} ${colors.gray}${provider.config.output}${colors.reset}`)
...
const generate = Command.make('generate', {
  ...
  Command.withHandler(
    Effect.fnUntraced(function* ({ provider, skipLint, skipFormat }) {
      ...
      yield* Console.log(`${colors.dim}Generating ${providers.length} provider(s)...${colors.reset}`)

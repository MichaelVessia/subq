...
export const toImplementation = (_importName: string, name: string, parsed: ParsedOpenApi): string => {
...
const buildSecurityRenderModel = (parsed: ParsedOpenApi): SecurityRenderModel => {
  ...
  function getOrSecurityMiddlewareName(schemes: ReadonlyArray<string>): string {
    ...
    const securityEntries = schemes
      .map((name) => `${JSON.stringify(name)}: ${schemeDeclarations.get(name)!}`)
      .join(', ')
...
const getSecurityMiddlewareBaseName = (schemes: ReadonlyArray<string>, joiner: 'And' | 'Or'): string => {

  ...
  const generate = Effect.fn(
    function* (spec: OpenAPISpec, options: OpenApiGenerateOptions) {
      ...
      const multipartSchemaRefs =
        options.format === 'httpapi' ? makeHttpApiMultipartSchemaRefs(spec.components?.schemas ?? {}) : undefined
  ...
  for (const [path, methods] of Object.entries(spec.paths)) {
    for (const method of methodNames) {
      ...
      const pathParameters =
        Predicate.isObject(methods) && Array.isArray((methods as any).parameters)
      ...
      if (Predicate.isNotUndefined(requestBody) && Predicate.isObject(requestBody)) {
        const content = Predicate.isObject(requestBody.content) ? (requestBody.content as Record<string, any>) : {}
      ...
      for (const [status, response] of resolvedResponses) {
        ...
        const content = Predicate.isObject(response.content) ? (response.content as Record<string, any>) : undefined

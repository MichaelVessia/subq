...
const format = Flag.choice('format', ['httpclient', 'httpclient-type-only', 'httpapi'] as const).pipe(
  Flag.withAlias('f'),
  Flag.withDescription('Output format to generate: httpclient | httpclient-type-only | httpapi (default: httpclient)'),
...
const root = Command.make('openapigen', { spec, format, name, patch }).pipe(
  Command.withHandler(
    Effect.fnUntraced(function* ({ name, spec, format, patch }) {
      ...
      yield* Effect.forEach(warnings, (warning) => Console.error(formatWarning(warning)), { discard: true })

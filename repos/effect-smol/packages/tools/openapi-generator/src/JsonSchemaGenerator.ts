...
export function make() {
  ...
  function generate(source: Source, components: JsonSchema.Definitions, typeOnly: boolean, options?: GenerateOptions) {
    ...
    if (typeOnly) {
      ...
      const recursiveInternalNames = makeRecursiveInternalNameMap(recursivelyForwardReferenced, [
  ...
  function generateHttpApi(source: Source, components: JsonSchema.Definitions, options?: GenerateHttpApiOptions) {
    const generated = makeCodeDocument(source, components, options)
    if (generated === undefined) {
      return ''
    }
  ...
  function makeCodeDocument(
    ...
    const definitions: JsonSchema.Definitions = Rec.map(components, (js) => fromSchemaOpenApi(source, js).schema)

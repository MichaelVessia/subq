...
const runCli = Effect.fnUntraced(function* (args: ReadonlyArray<string>) {
  const module = (yield* Effect.promise(() => import(new URL('../src/main.ts', import.meta.url).href))) as CliMainModule
...
describe('openapigen CLI', () => {
  ...
  it.effect('routes --format values and defaults to httpclient', () =>
    Effect.gen(function* () {
      ...
      const httpclientResult = yield* runCli(['--spec', spec, '--name', 'CliClient', '--format', 'httpclient'])
      const typeOnlyResult = yield* runCli(['--spec', spec, '--name', 'CliClient', '--format', 'httpclient-type-only'])
      const httpapiResult = yield* runCli(['--spec', spec, '--name', 'CliClient', '--format', 'httpapi'])

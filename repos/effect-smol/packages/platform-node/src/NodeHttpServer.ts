...
> => {
  ...
  return Effect.withFiber((parent) => {
    ...
    return Effect.succeed(function handler(nodeRequest: Http.IncomingMessage, nodeResponse: Http.ServerResponse) {
...
> => {
  ...
  return Effect.withFiber((parent) => {
    const services = parent.context
    return Effect.succeed(function handler(nodeRequest: Http.IncomingMessage, socket: Duplex, head: Buffer) {
...
> =>
  Layer.mergeAll(
    Layer.effect(HttpServer.HttpServer)(Effect.flatMap(Config.unwrap(options), (options) => make(evaluate, options))),
    layerHttpServices,
  )

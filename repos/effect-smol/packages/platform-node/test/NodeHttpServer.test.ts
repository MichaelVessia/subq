  ...
  describe('HttpServerRespondable', () => {
    it.effect('error/schema', () =>
      Effect.gen(function* () {
        ...
        yield* HttpRouter.add('GET', '/home', new CustomError({ name: 'test' })).pipe(HttpRouter.serve, Layer.build)
        const client = yield* HttpClient.HttpClient
        const res = yield* client.get('/home')
    ...
    it.effect('httpapi error', () =>
      Effect.gen(function* () {
        yield* HttpRouter.add('GET', '/home', new HttpApiError.BadRequest({})).pipe(HttpRouter.serve, Layer.build)
        const client = yield* HttpClient.HttpClient
        const res = yield* client.get('/home')

...
describe('HttpApi', () => {
  ...
  describe('middleware', () => {
    it.effect('error', () => {
      class M extends HttpApiMiddleware.Service<M>()('Http/Logger', {
        error: Schema.String.pipe(HttpApiSchema.status('MethodNotAllowed'), HttpApiSchema.asText()),
      }) {}
      ...
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) => handlers.handle('a', () => Effect.succeed(1)))
      const MLive = Layer.succeed(M, () => Effect.fail('error'))

      const ApiLive = HttpRouter.serve(HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLive), Layer.provide(MLive)), {
        disableListenLog: true,
        disableLogger: true,
      }).pipe(Layer.provideMerge(NodeHttpServer.layerTest))
    ...
    it.effect('error array', () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) =>
        handlers.handle('unauthorized', () => Effect.succeed('ok')).handle('forbidden', () => Effect.succeed('ok')),
      )
    ...
    it.effect('client middleware modifies request and receives metadata', () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) =>
        handlers.handle('a', (ctx) => Effect.succeed(ctx.request.headers['x-client'] ?? 'missing')),
      )
      ...
      return Effect.gen(function* () {
        ...
        const client = yield* HttpApiClient.make(Api).pipe(Effect.provide(MClient))
    ...
    it.effect('client middleware chain order is LIFO', () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) =>
        handlers.handle('a', (ctx) =>
          Effect.succeed(`${ctx.request.headers['x-m1'] ?? '0'}${ctx.request.headers['x-m2'] ?? '0'}`),
        ),
      )
      ...
      return Effect.gen(function* () {
        ...
        const client = yield* HttpApiClient.make(Api).pipe(Effect.provide([M1Client, M2Client]))
    ...
    it.effect('required client middleware can fail with typed clientError', () => {
      ...
      return Effect.gen(function* () {
        const MClient = HttpApiMiddleware.layerClient(M, () => Effect.fail(new ClientFailure({})))
    ...
    it.effect('optional client middleware is skipped when not provided', () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) =>
        handlers.handle('a', (ctx) => Effect.succeed(ctx.request.headers['x-optional'] ?? 'missing')),
      )
      const MLive = Layer.succeed(M, (effect) => effect)
    ...
    it.effect('layerClient supports effectful construction', () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) =>
        handlers.handle('a', (ctx) => Effect.succeed(ctx.request.headers['x-effect'] ?? 'missing')),
      )
      const MLive = Layer.succeed(M, (effect) => effect)
    ...
    it.effect('security middleware can be implemented with layerClient', () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) => handlers.handle('a', () => CurrentToken))
    ...
    it('security middleware cache does not reuse the first service impl', async () => {
      ...
      const makeWebHandler = (marker: string) => {
        const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) =>
          handlers.handle('a', () => Effect.map(CurrentMarker, (marker) => ({ marker }))),
        )
        const MLive = Layer.succeed(M)({
    ...
    it.effect('addHttpApi + middleware works across merged groups', () => {
      ...
      const M1Live = Layer.succeed(M1, (effect, { endpoint, group }) =>
        Effect.sync(() => calls.push(`m1:${group.identifier}.${endpoint.name}`)).pipe(Effect.flatMap(() => effect)),
      )
      const M2Live = Layer.succeed(M2, (effect, { endpoint, group }) =>
        Effect.sync(() => calls.push(`m2:${group.identifier}.${endpoint.name}`)).pipe(Effect.flatMap(() => effect)),
      )
      ...
      return Effect.gen(function* () {
        ...
        assert.deepStrictEqual(calls, ['m2:health.health', 'm1:health.health', 'm2:users.list', 'm1:users.list'])
      }).pipe(Effect.provide(ApiLive))
    })
  ...
  it.effect('missing addHttpApi group layer has actionable error', () => {
    ...
    const ApiLive = HttpRouter.serve(HttpApiBuilder.layer(Api).pipe(Layer.provide(UsersLive)), {
      disableListenLog: true,
      disableLogger: true,
    }).pipe(Layer.provideMerge(NodeHttpServer.layerTest))
  ...
  describe('success option', () => {
    ...
    it('no content via toWebHandler', async () => {
      const Api = HttpApi.make('api').add(
        HttpApiGroup.make('group').add(
          HttpApiEndpoint.delete('remove', '/items/:id', { params: { id: Schema.String } }),
        ),
      )
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) => handlers.handle('remove', () => Effect.void))
      const ApiLive = HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLive), Layer.provide(HttpServer.layerServices))
  ...
  describe('original tests', () => {
    ...
    describe('errors', () => {
      ...
      it.effect('empty errors decode', () =>
        Effect.gen(function* () {
          const client = yield* HttpApiClient.make(Api)
          const error = yield* client.groups.findById({ params: { id: 0 } }).pipe(Effect.flip)
      ...
      it.effect('BadRequest', () =>
        Effect.gen(function* () {
          const client = yield* HttpApiClient.make(Api)
          const error = yield* client.users.upload({ params: {}, payload: new FormData() }).pipe(Effect.flip)
    ...
    it.effect('client responseMode response-only skips decoding', () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, 'group', (handlers) =>
        handlers.handleRaw('bad', () => Effect.succeed(HttpServerResponse.text('not-json'))),
      )
      ...
      const ApiLive = HttpApiBuilder.layer(Api).pipe(
        Layer.provide(
          HttpApiBuilder.group(Api, 'group', (handlers) =>
            handlers.handle('error', () => new RateLimitError({ message: 'Rate limit exceeded' })),
          ),
        ),
        HttpRouter.serve,
  ...
  .add(
    ...
    HttpApiEndpoint.post('create', '/', {
      payload: [
        ...
        Schema.Struct({ foo: Schema.String }).pipe(HttpApiSchema.asFormUrlEncoded()),
        Schema.Struct(Struct.pick(Group.fields, ['name'])).pipe(HttpApiSchema.asMultipart()),
      ],
      success: Group,
    }),
...
const HttpGroupsLive = HttpApiBuilder.group(Api, 'groups', (handlers) =>
  handlers
    .handle('findById', ({ params }) =>
      params.id === 0 ? Effect.fail(new GroupError({})) : Effect.succeed(new Group({ id: 1, name: 'foo' })),
    )
...
const HttpLive = HttpRouter.serve(HttpApiLive, {
  disableListenLog: true,
  disableLogger: true,
}).pipe(Layer.provideMerge(NodeHttpServer.layerTest), Layer.provide(DateTime.layerCurrentZoneOffset(0)))

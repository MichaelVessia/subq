...
const staticFilesLayer = Layer.mergeAll(NodePathLayer.layer, NodeFileSystem.layer, NodeHttpPlatform.layer)

const makeHandler = (root: string, options: Omit<Parameters<typeof HttpStaticServer.make>[0], 'root'> = {}) =>
...
const withStaticFiles = async (
  ...
  await Promise.all([cp(fixturesRoot, root, { recursive: true }), copyFile(fixturesOutsideFile, outsideFile)])
    ...
    await withStaticFiles(
      async ({ handler }) => {
        ...
        const any = await handler(
          new Request('http://localhost/conditional.txt', { headers: { 'If-None-Match': '*' } }),
        )
  ...
  it('rejects directory traversal attempts', async () => {
    await withStaticFiles(async ({ handler, outsideFile }) => {
      ...
      assert.deepStrictEqual(
        [plainTraversal.status, encodedTraversal.status, encodedExistingParentFileTraversal.status],
        [404, 404, 404],
      )
    })
  })

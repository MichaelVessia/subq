...
const simpleCmd = Command.make('greet', {
  ...
  loud: Flag.boolean('loud').pipe(Flag.withAlias('l'), Flag.withDescription('Shout the greeting')),
  times: Flag.integer('times').pipe(Flag.withDescription('Repeat count'), Flag.withDefault(1)),
}).pipe(Command.withDescription('Greet someone'))

const withSubcommands = (() => {
  const start = Command.make('start', {
    port: Flag.integer('port').pipe(Flag.withAlias('p'), Flag.withDescription('Port number')),
  ...
  const stop = Command.make('stop', {
    force: Flag.boolean('force').pipe(Flag.withAlias('f'), Flag.withDescription('Force stop')),
  }).pipe(Command.withDescription('Stop the server'))

  return Command.make('server', {
    verbose: Flag.boolean('verbose').pipe(Flag.withAlias('v')),
  }).pipe(Command.withDescription('Server management'), Command.withSubcommands([start, stop]))
})()

const withChoices = Command.make('deploy', {
  env: Flag.choice('env', ['dev', 'staging', 'prod']).pipe(Flag.withDescription('Target environment')),
...
const withPaths = Command.make('process', {
  ...
  source: Argument.file('source', { mustExist: false }).pipe(Argument.withDescription('Source file')),
...
const emptyCmd = Command.make('noop').pipe(Command.withDescription('Does nothing'))
...
describe('Fish completions', () => {
  ...
  it('guards nested child subcommands against re-offering', () => {
    ...
    assert.include(script, '__fish_seen_subcommand_from sub; and not __fish_seen_subcommand_from action')
  })

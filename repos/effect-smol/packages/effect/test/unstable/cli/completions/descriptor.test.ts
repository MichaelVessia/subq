...
describe('CommandDescriptor', () => {
  describe('fromCommand', () => {
    it('extracts command name and description', () => {
      const cmd = Command.make('deploy').pipe(Command.withDescription('Deploy the application'))
    ...
    it('extracts nested subcommands recursively', () => {
      const leaf = Command.make('leaf').pipe(Command.withDescription('A leaf command'))
    ...
    it('extracts descriptions from flags and arguments', () => {
      const cmd = Command.make('test', {
        ...
        host: Argument.string('host').pipe(Argument.withDescription('Hostname')),
      })
      const desc = fromCommand(cmd)
    ...
    it('handles deeply nested command trees', () => {
      ...
      const level2 = Command.make('level2').pipe(Command.withSubcommands([level3]))
      const level1 = Command.make('level1').pipe(Command.withSubcommands([level2]))
    ...
    it('extracts multiple subcommands', () => {
      ...
      const root = Command.make('root').pipe(Command.withSubcommands([a, b, c]))
      const desc = fromCommand(root)
    ...
    it('prefers short descriptions for subcommand listings', () => {
      ...
      const test = Command.make('test').pipe(Command.withDescription('Run the full test suite'))

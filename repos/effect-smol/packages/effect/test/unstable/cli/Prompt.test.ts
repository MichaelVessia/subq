...
const toRawFrames = (lines: ReadonlyArray<unknown>) =>
  lines.map((line) => String(line)).filter((line) => stripAnsi(line).split(bell).join('').trim().length > 0)
  ...
  const FilePromptLayer = Layer.mergeAll(
    ...
    FileSystem.layerNoop({
      ...
      readDirectory: (directory) =>
        Effect.succeed(directory === '/workspace' ? ['alpha.txt', 'banana.txt', 'basket.txt'] : []),
      stat: (path) =>
        Effect.succeed(path.endsWith('.txt') ? ({ type: 'File' } as any) : ({ type: 'Directory' } as any)),
    }),
    PathLayer,
    TerminalLayer,
  )

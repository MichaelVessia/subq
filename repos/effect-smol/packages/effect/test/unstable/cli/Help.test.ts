...
describe('Command help output', () => {
  ...
  it.effect('aligns flag descriptions when flag names are long', () =>
    Effect.gen(function* () {
      const command = Command.make('tool', {
        ...
        veryLong: Flag.string('this-is-a-very-very-long-flag-name').pipe(Flag.withDescription('Long flag description')),
      })
  ...
  it.effect('grouped subcommands', () =>
    Effect.gen(function* () {
      const ungrouped = Command.make('ungrouped').pipe(Command.withDescription('This command is not in a group'))

  ...
  describe('annotations', () => {
    ...
    it.effect('should keep annotations when adding subcommands', () =>
      Effect.gen(function* () {
        ...
        const command = Command.make('root').pipe(Command.annotate(Scope, 'root'), Command.withSubcommands([child]))
  ...
  describe('run', () => {
    ...
    it.effect('should reject global flags that are out of scope for selected subcommand', () =>
      Effect.gen(function* () {
        ...
        yield* runCommand(['db', '--region', 'eu-west-1']).pipe(Effect.ignore)
    ...
    it.effect('should show only path-active global flags in subcommand help', () =>
      Effect.gen(function* () {
        ...
        const deploy = Command.make('deploy', {}, () => Effect.void).pipe(Command.withGlobalFlags([Region]))
  ...
  describe('withSubcommands', () => {
    ...
    it.effect('should handle boolean shared flags before subcommands', () =>
      Effect.gen(function* () {
        ...
        yield* runCommand(['--config', 'prod.json', '--verbose', 'deploy', '--target-version', '1.0.0'])
    ...
    it.effect('should support optional boolean flags and --no-<flag> negation', () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(captured, [Option.none(), Option.some(true), Option.some(false)])
      }).pipe(Effect.provide(TestLayer)),
    )
    ...
    it.effect('should use short descriptions in subcommand listings', () =>
      Effect.gen(function* () {
        ...
        const withFallbackDescription = Command.make('test').pipe(Command.withDescription('Run the full test suite'))
        const root = Command.make('tool').pipe(Command.withSubcommands([withShortDescription, withFallbackDescription]))
    ...
    it.effect('should preserve examples when adding subcommands', () =>
      Effect.gen(function* () {
        const root = Command.make('root').pipe(
          Command.withExamples([{ command: 'myapp root', description: 'Run root command' }]),
          Command.withSubcommands([Command.make('child')]),
        )

        const helpDoc = toImpl(root).buildHelpDoc(['root'])

        assert.deepStrictEqual(helpDoc.examples, [{ command: 'myapp root', description: 'Run root command' }])
      }),
    )
  })
    ...
    it.effect('should include flag choices in help doc descriptions', () =>
      Effect.gen(function* () {
        const command = Command.make('tool', {
          mode: Flag.choice('mode', ['dev', 'prod']).pipe(Flag.withDescription('Execution mode')),

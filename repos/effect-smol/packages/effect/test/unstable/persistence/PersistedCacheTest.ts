  ...
  describe(`PersistedCache (${storeId})`, { timeout: 30_000 }, () => {
    it.effect('smoke test', () =>
      Effect.gen(function* () {
        ...
        Effect.catchFilter(
          (e) => (e instanceof TransientError ? Result.succeed(e) : Result.fail(e)),
          () => Effect.void,
        ),
        flakyTest,
      ),
    ...
    it.effect("requireServicesAt: 'lookup' requires lookup services at get-time", () =>
      Effect.gen(function* () {
        ...
        const result1 = yield* cache
          ...
          .pipe(Effect.provideService(LookupService, LookupService.of({ value: 'first' })))
        const result2 = yield* cache
          .get(new TTLRequest({ id: 2 }))
          .pipe(Effect.provideService(LookupService, LookupService.of({ value: 'second' })))
        const result3 = yield* cache
          .get(new TTLRequest({ id: 1 }))
          .pipe(Effect.provideService(LookupService, LookupService.of({ value: 'third' })))

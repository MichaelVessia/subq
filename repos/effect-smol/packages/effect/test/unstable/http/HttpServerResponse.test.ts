...
describe('HttpServerResponse', () => {
  ...
  it.effect('fromClientResponse preserves stream requirements', () =>
    Effect.gen(function* () {
      ...
      const text = yield* roundTrip.text.pipe(Effect.provideService(References.CurrentConcurrency, 420))
  ...
  it.effect('fromClientResponse preserves formData bodies', () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(response.headers['content-type']?.startsWith('multipart/form-data; boundary='), true)

  ...
  describe('logger', () => {
    it.effect('logs only the request path in http.url', () =>
      Effect.gen(function* () {
        ...
        yield* HttpMiddleware.logger(Effect.succeed(HttpServerResponse.empty({ status: 204 }))).pipe(
    ...
    it.effect('uses a stable http.span log span name', () =>
      Effect.gen(function* () {
        ...
        const loggedApp = HttpMiddleware.logger(Effect.succeed(HttpServerResponse.empty({ status: 204 }))).pipe(
          Effect.provide(Logger.layer([logger])),
        )

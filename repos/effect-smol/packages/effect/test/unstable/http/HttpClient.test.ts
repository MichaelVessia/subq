  ...
  describe('stream', () => {
    it.effect('aborts the request when the response stream ends early', () =>
      Effect.gen(function* () {
        ...
        const chunks = yield* response.stream.pipe(Stream.take(5), Stream.runCollect)
  ...
  describe('withRateLimiter', () => {
    ...
    it.effect('inspects remaining headers to infer updated limits', () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* client
          .get('http://test/')
          .pipe(Effect.andThen(client.get('http://test/')), Effect.forkChild({ startImmediately: true }))

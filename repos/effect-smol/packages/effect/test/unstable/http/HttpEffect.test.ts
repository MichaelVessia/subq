...
describe('Http/App', () => {
  describe('toWebHandler', () => {
    ...
    test('stream runtime', async () => {
      ...
      ).pipe(HttpEffect.toWebHandlerWith(References.CurrentConcurrency.context(420)))
      const response = await handler(new Request('http://localhost:3000/'))
    ...
    test('stream layer', async () => {
      const { handler } = HttpEffect.toWebHandlerLayer(
        Effect.succeed(
          HttpServerResponse.stream(
            References.CurrentConcurrency.pipe(Stream.fromEffect, Stream.map(String), Stream.encodeText),
          ),
        ),

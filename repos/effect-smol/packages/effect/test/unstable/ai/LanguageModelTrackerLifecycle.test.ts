      ...
      const languageModel = yield* LanguageModel.make({
        generateText: (options) =>
          Effect.sync(() => {
            ...
            return [responseMetadataPart(responseId), { type: 'text', text: `assistant-${responseId}` }, finishPart]
          }),
      ...
      const languageModel = yield* LanguageModel.make({
        generateText: (options) =>
          Effect.sync(() => {
            ...
            responseIndex += 1
            providerOptions.push(options)
            return [responseMetadataPart(responseId), { type: 'text', text: '{"count":1}' }, finishPart]
          }),
        streamText: () => Stream.empty,
      })
    ...
    Effect.gen(function* () {
      ...
      const languageModel = yield* LanguageModel.make({
        ...
        streamText: (options) => {
          ...
          responseIndex += 1
          providerOptions.push(options)
          return Stream.fromIterable([responseMetadataPart(responseId), finishPart])
        },
      })
      ...
      const languageModel = yield* LanguageModel.make({
        generateText: (options) =>
          Effect.sync(() => {
            ...
            return [responseMetadataPart('resp_none'), { type: 'text', text: '{"count":1}' }, finishPart]
          }),
        streamText: (options) => {
          streamTextOptions.push(options)
          return Stream.fromIterable([responseMetadataPart('resp_none_stream'), finishPart])
        },
      })

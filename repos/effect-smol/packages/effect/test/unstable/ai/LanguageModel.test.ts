  ...
  describe('provider options', () => {
    ...
    it('falls back to full prompt in generateText when incremental request fails', () =>
      Effect.gen(function* () {
        ...
        const incrementalPrompt = Prompt.make([Prompt.userMessage({ content: [Prompt.textPart({ text: 'next' })] })])
  ...
  describe('tool approval', () => {
    ...
    it('strips denied approval artifacts from prompt sent to provider', () =>
      Effect.gen(function* () {
        ...
        if (lastMessage.role === 'tool') {
          const toolResults = lastMessage.content.filter((p): p is Prompt.ToolResultPart => p.type === 'tool-result')
          strictEqual(toolResults.length, 1)
          strictEqual(toolResults[0].isFailure, true)
        }
      }))
    ...
    it('strips only resolved approvals, preserves unrelated parts', () =>
      Effect.gen(function* () {
        ...
        assertDefined(assistantMsg)
        if (assistantMsg.role === 'assistant') {
          const approvalRequests = assistantMsg.content.filter((p) => p.type === 'tool-approval-request')

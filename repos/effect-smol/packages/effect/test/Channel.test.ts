...
describe('Channel', () => {
  ...
  describe('conditional catch', () => {
    ...
    it.effect('catchFilter with Filter', () =>
      Effect.gen(function* () {
        const filter = Filter.make((error: string) => (error === 'boom' ? Result.succeed(error) : Result.fail(error)))
        const result = yield* Channel.fail('boom').pipe(
    ...
    it.effect('catchCauseFilter with Filter', () =>
      Effect.gen(function* () {
        const result = yield* Channel.fail('boom').pipe(
          Channel.catchCauseFilter(Cause.findError as any, (error) => Channel.succeed(`recovered: ${error}`)),
          Channel.runCollect,
        )
        assert.deepStrictEqual(result, ['recovered: boom'])
      }),
    ...
    it.effect('catchReason orElse', () =>
      Effect.gen(function* () {
        const result = yield* Channel.fail(new AiError({ reason: new QuotaExceededError({ limit: 100 }) })).pipe(
          Channel.catchReason(
            'AiError',
            'RateLimitError',
    ...
    it.effect('catchReasons orElse', () =>
      Effect.gen(function* () {
        const result = yield* Channel.fail(new AiError({ reason: new RateLimitError({ retryAfter: 60 }) })).pipe(

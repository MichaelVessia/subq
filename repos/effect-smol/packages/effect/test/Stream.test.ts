...
describe('Stream', () => {
  ...
  describe('constructors', () => {
    ...
    it.effect('fromIterable - emits array as one chunk by default', () =>
      Effect.gen(function* () {
        const result = yield* Stream.fromIterable([1, 2, 3, 4]).pipe(Stream.chunks, Stream.runCollect)
  ...
  describe('encoding', () => {
    it.effect('decodeText handles multi-byte characters split across chunks', () =>
      Effect.gen(function* () {
        ...
        const result = yield* Stream.make(bytes.slice(0, 2), bytes.slice(2)).pipe(Stream.decodeText(), Stream.mkString)
      ...
      it.effect('does not pull upstream again after flushing the final line', () =>
        Effect.gen(function* () {
          ...
          const stream = Stream.fromPull(
            Effect.sync(() => {
              const pull = Effect.suspend((): Pull.Pull<Array.NonEmptyReadonlyArray<string>, string> => {
                pulls++
                switch (pulls) {
                  case 1:
          ...
          const result = yield* stream.pipe(Stream.splitLines, Stream.runCollect)
  ...
  describe('error handling', () => {
    ...
    describe('catchReason', () => {
      ...
      it.effect('catches matching reason - handler succeeds', () =>
        Effect.gen(function* () {
          const result = yield* Stream.fail(new AiError({ reason: new RateLimitError({ retryAfter: 60 }) })).pipe(
      ...
      it.effect('handles non-matching reason with orElse', () =>
        Effect.gen(function* () {
          const result = yield* Stream.fail(new AiError({ reason: new QuotaExceededError({ limit: 100 }) })).pipe(
            Stream.catchReason(
              'AiError',
              'RateLimitError',
  ...
  describe('flattening', () => {
    it.effect('flatten supports dropping parens in pipe', () =>
      Effect.gen(function* () {
        const result = yield* pipe(Stream.make(Stream.make(1, 2), Stream.make(3, 4)), Stream.flatten, Stream.runCollect)
        assert.deepStrictEqual(result, [1, 2, 3, 4])
      }),
    )
  ...
  describe('aggregateWithin', () => {
    ...
    it.effect('groupedWithin flushes final partial batch on upstream end without waiting for the schedule', () =>
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4, 5).pipe(Stream.groupedWithin(3, '1 second'), Stream.runCollect)
    ...
    it.effect('fails fast', () =>
      Effect.gen(function* () {
        ...
        yield* pipe(
          Stream.range(1, 9),
          Stream.tap((n) =>
            pipe(Effect.fail('Boom'), Effect.when(Effect.sync(() => n === 6)), Effect.andThen(Queue.offer(queue, n))),
          ),
          Stream.aggregateWithin(
      ...
      it.effect('waits for both streams to emit before producing output', () =>
        Effect.gen(function* () {
          ...
          const fiber = yield* Effect.forkScoped(
            Stream.zipLatest(Stream.fromQueue(queue1), Stream.fromQueue(queue2)).pipe(
              Stream.take(3),
              Stream.runCollect,
            ),
          )
  ...
  describe('merge', () => {
    it.effect('data-first with options', () =>
      Effect.gen(function* () {
        const result = yield* Stream.runCollect(Stream.merge(Stream.make(1), Stream.never, { haltStrategy: 'left' }))
        deepStrictEqual(result, [1])
      }),
    )
              ...
              const fiber = yield* pipe(
                ...
                Stream.tap((n) =>
                  pipe(
                    ...
                    Effect.andThen(pipe(Deferred.succeed(latch, void 0), Effect.when(Effect.sync(() => n === 2)))),
                  ),
                ),
                Stream.runDrain,
                Effect.forkChild,
  ...
  describe('peel', () => {
    ...
    it.effect('peel - propagates errors', () =>
      Effect.gen(function* () {
        ...
        const sink = Sink.fold<ReadonlyArray<number>, number>(Array.empty, constTrue, (acc, n) =>
          Effect.succeed(Array.append(acc, n)),
        )
    ...
    it.effect('timeoutOrElse - should not apply timeout after switch', () =>
      Effect.gen(function* () {
        const fiber = yield* pipe(
          ...
          Stream.timeoutOrElse({
            duration: Duration.zero,
            orElse: () => Stream.succeed(1).pipe(Stream.tap(() => Effect.sleep(Duration.seconds(1)))),
          }),
          Stream.runCollect,
          Effect.forkChild({ startImmediately: true }),
        )

...
describe('Sink', () => {
  ...
  describe('takeWhile', () => {
    it.effect('takeWhile', () =>
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4).pipe(Stream.run(Sink.takeWhile((n) => n < 3)))
    ...
    it.effect('takeWhileFilterEffect', () =>
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4).pipe(
          Stream.run(
            Sink.takeWhileFilterEffect((n) => Effect.succeed(n < 3 ? Result.succeed(n + 1) : Result.failVoid)),
          ),
        )

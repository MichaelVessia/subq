...
describe('Optic generation', () => {
  ...
  describe('toIso', () => {
    ...
    it('NonEmptyArray', () => {
      ...
      deepStrictEqual(
        modify([Value.make({ a: new Date(0) }), Value.make({ a: new Date(1) }), Value.make({ a: new Date(2) })]),
        [Value.make({ a: new Date(1) }), Value.make({ a: new Date(3) }), Value.make({ a: new Date(4) })],
      )
    })
    ...
    it('StructWithRest', () => {
      ...
      deepStrictEqual(modify({ a: Value.make({ a: new Date(0) }), b: Value.make({ a: new Date(1) }) }), {
        a: Value.make({ a: new Date(1) }),
    ...
    it('Option', () => {
      ...
      assertSome(modify(Option.some(Value.make({ a: new Date(0) }))), Value.make({ a: new Date(1) }))
    ...
    it('Cause', () => {
      ...
      deepStrictEqual(modify(Cause.fail(Value.make({ a: new Date(0) }))), Cause.fail(Value.make({ a: new Date(1) })))
    })
    ...
    it('ReadonlySet', () => {
      ...
      deepStrictEqual(modify(new Set([Value.make({ a: new Date(0) })])), new Set([Value.make({ a: new Date(1) })]))
    })

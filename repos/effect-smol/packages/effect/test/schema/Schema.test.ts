...
Expected a value between -2147483648 and 2147483647, got 9007199254740992`,
    ...
    describe('Record checks', () => {
      it('isMinProperties', async () => {
        ...
        await decoding.fail({}, `Expected a value with at least 1 entry, got {}`)
      })
      ...
      it('isMaxProperties', async () => {
        ...
        const decoding = asserts.decoding()
        await decoding.succeed({ a: 1, b: 2 })
        await decoding.fail({ a: 1, b: 2, c: 3 }, `Expected a value with at most 2 entries, got {"a":1,"b":2,"c":3}`)
        await decoding.fail({ a: 1, b: 2, c: 3 }, `Expected a value with at most 2 entries, got {"a":1,"b":2,"c":3}`)
      })
      ...
      it('isMinProperties with symbol keys', async () => {
        ...
        await decoding.fail({ [sym]: 1 }, `Expected a value with at least 2 entries, got {Symbol(test):1}`)
      })
      ...
      it('isPropertiesLengthBetween', async () => {
        ...
        await decoding.fail({ a: 1 }, `Expected a value with exactly 2 entries, got {"a":1}`)
        await decoding.fail({ a: 1, b: 2, c: 3 }, `Expected a value with exactly 2 entries, got {"a":1,"b":2,"c":3}`)
      })
      ...
      it('isPropertiesLengthBetween with symbol keys', async () => {
        ...
        await decoding.fail({ [sym1]: 1 }, `Expected a value with exactly 2 entries, got {Symbol(test1):1}`)
        await decoding.fail(
          { [sym1]: 1, [sym2]: 2, a: 3 },
...
Expected a value with a size of at most 2, got Map([["a",1],["b",NaN],["c",3]])`,
  ...
  describe('withConstructorDefault', () => {
    ...
    describe('Tuple', () => {
      it('Tuple & Some', async () => {
        const schema = Schema.Tuple([Schema.FiniteFromString.pipe(Schema.withConstructorDefault(Effect.succeed(-1)))])
        const asserts = new TestSchema.Asserts(schema)
      ...
      it('inner defaults (Tuple)', async () => {
        const schema = Schema.Tuple([
          Schema.Tuple([Schema.FiniteFromString.pipe(Schema.withConstructorDefault(Effect.succeed(-1)))]),
        ])
        const asserts = new TestSchema.Asserts(schema)
      ...
      it('nested defaults (Tuple)', async () => {
        const schema = Schema.Tuple([
          Schema.Tuple([Schema.FiniteFromString.pipe(Schema.withConstructorDefault(Effect.succeed(-1)))]).pipe(
  ...
  describe('BigDecimal checks', () => {
    it('isGreaterThanBigDecimal', async () => {
      ...
      await decoding.fail(BigDecimal.fromStringUnsafe('1'), `Expected a value greater than 1, got BigDecimal(1)`)
    })

    it('isGreaterThanOrEqualToBigDecimal', async () => {
      const schema = Schema.BigDecimal.check(Schema.isGreaterThanOrEqualToBigDecimal(BigDecimal.fromStringUnsafe('1')))
      const asserts = new TestSchema.Asserts(schema)
    ...
    it('isLessThanBigDecimal', async () => {
      ...
      await decoding.fail(BigDecimal.fromStringUnsafe('1'), `Expected a value less than 1, got BigDecimal(1)`)
    })
    ...
    it('isBetweenBigDecimal', async () => {
      ...
      await decoding.fail(BigDecimal.fromStringUnsafe('0'), `Expected a value between 1 and 5, got BigDecimal(0)`)
    })
  })
  ...
  describe('annotateEncoded', () => {
    it('non-transforming schema', () => {
      const schema = Schema.String.pipe(Schema.annotateEncoded({ title: 'encoded title' }))
      strictEqual(SchemaAST.toEncoded(schema.ast).annotations?.title, 'encoded title')
    })

    it('transforming schema', () => {
      const schema = Schema.NumberFromString.pipe(Schema.annotateEncoded({ title: 'encoded title' }))
      strictEqual(SchemaAST.toEncoded(schema.ast).annotations?.title, 'encoded title')
    })
  })
...
describe('resolveAnnotations', () => {
  ...
  it('returns annotations from the last check', () => {
    const schema = Schema.String.annotate({ title: 'base' }).check(Schema.isNonEmpty().annotate({ title: 'check' }))

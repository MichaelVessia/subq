...
describe('RpcSerialization', () => {
  ...
  it('jsonRpc encodes batched responses as an array', () => {
    ...
    const encoded = parser.encode([responseExitSuccess('1', 'one'), responseExitSuccess('2', 'two')])
    assert(encoded !== undefined)
  ...
  it('jsonRpc preserves id 0 across decode and encode', () => {
    ...
    assert.strictEqual(encoded, '{"jsonrpc":"2.0","method":"users.get","params":null,"id":0,"headers":[]}')
  })
  ...
  it('jsonRpc preserves empty string id across decode and encode', () => {
    ...
    assert.strictEqual(encoded, '{"jsonrpc":"2.0","method":"users.get","params":null,"id":"","headers":[]}')
  })

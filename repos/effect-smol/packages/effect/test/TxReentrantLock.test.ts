  ...
  describe('withLock operations', () => {
    ...
    it.effect('withReadLock releases on failure', () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.exit(TxReentrantLock.withReadLock(lock, Effect.fail('boom')))
    ...
    it.effect('withWriteLock releases on failure', () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.exit(TxReentrantLock.withWriteLock(lock, Effect.fail('boom')))
    ...
    it.effect('withReadLock data-last (pipe) style', () =>
      Effect.gen(function* () {
        ...
        const result = yield* lock.pipe(TxReentrantLock.withReadLock(Effect.succeed('piped')))
    ...
    it.effect('withWriteLock data-last (pipe) style', () =>
      Effect.gen(function* () {
        ...
        const result = yield* lock.pipe(TxReentrantLock.withWriteLock(Effect.succeed('piped-write')))
  ...
  describe('concurrency', () => {
    it.effect('multiple readers can proceed concurrently', () =>
      Effect.gen(function* () {
        ...
        const fiber1 = yield* Effect.forkChild(TxReentrantLock.withReadLock(lock, Effect.succeed(1)))
        const fiber2 = yield* Effect.forkChild(TxReentrantLock.withReadLock(lock, Effect.succeed(2)))
        const fiber3 = yield* Effect.forkChild(TxReentrantLock.withReadLock(lock, Effect.succeed(3)))

        const [r1, r2, r3] = yield* Effect.all([Fiber.join(fiber1), Fiber.join(fiber2), Fiber.join(fiber3)])

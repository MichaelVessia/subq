...
describe('Config', () => {
  it('a config is an Effect and can be yielded', () => {
    ...
    const result = Effect.runSync(
      Effect.provide(Config.schema(Schema.Struct({ STRING: Schema.String })), ConfigProvider.layer(provider)),
    )
  ...
  at ["b"]["a"]`,
        ...
        it('name + nested + nested', async () => {
          ...
          await assertSuccess(config, ConfigProvider.fromUnknown({ c: { b: { a: 'value' } } }), 'value')
          await assertFailure(
            config,
  ...
  at ["c"]["b"]["a"]`,
        ...
        it('all', async () => {
          ...
          await assertSuccess(config, ConfigProvider.fromUnknown({ database: { host: 'localhost', port: '5432' } }), {
            host: 'localhost',
            port: 5432,
          })
          await assertFailure(

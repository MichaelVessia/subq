...
describe.sequential('Atom', () => {
  ...
  it('stream empty produces NoSuchElementError', async () => {
    const atom = Atom.make(Stream.empty) satisfies Atom.Atom<AsyncResult.AsyncResult<never, Cause.NoSuchElementError>>
    const r = AtomRegistry.make()
    const cancel = r.mount(atom)
    ...
    assert(AsyncResult.isFailure(result))
    assert.deepStrictEqual(AsyncResult.error(result), Option.some(new Cause.NoSuchElementError()))
  ...
  test(`swr refresh is forceful while fresh`, () => {
    const r = AtomRegistry.make()
    let runs = 0
    const atom = Atom.make(Effect.sync(() => ++runs)).pipe(Atom.swr({ staleTime: 1_000 }))
    const unmount = r.mount(atom)
  ...
  test(`swr keeps previous value while stale revalidation runs`, async () => {
    const r = AtomRegistry.make()
    let runs = 0
    const atom = Atom.make(Effect.sync(() => ++runs).pipe(Effect.delay(50))).pipe(Atom.swr({ staleTime: 100 }))
    const unmount = r.mount(atom)
  ...
  test(`swr revalidateOnMount false skips first read only`, () => {
    ...
    const atom = Atom.make(() => {
      runs++
      return Effect.fail('fail')
    }).pipe(Atom.swr({ staleTime: 1_000, revalidateOnMount: false, revalidateOnFocus: true, focusSignal }))
    const unmount = r.mount(atom)
  ...
  test(`swr does not refresh on focus signal when disabled or omitted`, () => {
...

    {
      ...
      const emitFocus = () => r.set(focusSignal, ++focus)
      let runs = 0
      const atom = Atom.make(Effect.sync(() => ++runs)).pipe(Atom.swr({ staleTime: 10_000, focusSignal }))
      const unmount = r.mount(atom)
  ...
  test(`swr uses registry initial values as source state`, () => {
    ...
    const atom = source.pipe(Atom.swr({ staleTime: 1_000, revalidateOnMount: false }), Atom.keepAlive)

...
describe('Effect', () => {
    ...
    it.effect('tuple result mode', () =>
      Effect.gen(function* () {
        ...
        const results = (yield* Effect.all(
          ...
          { mode: 'result' },
        )) satisfies [Result.Result<number, never>, Result.Result<never, string>, Result.Result<number, never>]
        assert.deepStrictEqual(executed, [1, 2, 3])
        assert.deepStrictEqual(results, [Result.succeed(1), Result.fail('boom'), Result.succeed(3)])
      }),
    )
    ...
    it.effect('awaits children forked by the wrapped effect', () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* Effect.gen(function* () {
          ...
          return 1
        }).pipe(Effect.awaitAllChildren, Effect.forkChild({ startImmediately: true }))
  ...
  describe('catchCauseIf with predicate', () => {
    it.effect('predicate match recovers', () =>
      Effect.gen(function* () {
        const result = yield* Effect.catchCauseIf(Effect.fail('e1'), Cause.hasFails, (cause) =>
          Effect.succeed(`recovered: ${Cause.squash(cause)}`),
        )
        assert.deepStrictEqual(result, 'recovered: e1')
      }),
    )
    it.effect('predicate no match preserves error', () =>
      Effect.gen(function* () {
        const result = yield* Effect.exit(
          Effect.catchCauseIf(Effect.fail('e1' as const), constFalse, () => Effect.succeed('recovered')),
        )
        assert.deepStrictEqual(result, Exit.fail('e1'))
      }),
    )
  ...
  describe('tapCauseIf', () => {
    ...
    it.effect('predicate no match skips tap', () =>
      Effect.gen(function* () {
        const tapped: Array<string> = []
        const result = yield* Effect.exit(
          Effect.tapCauseIf(Effect.fail('e1'), constFalse, () => Effect.sync(() => tapped.push('tapped'))),
        )
        assert.deepStrictEqual(tapped, [])
        assert.deepStrictEqual(result, Exit.fail('e1'))
      }),
    ...
    it.effect('success skips tap', () =>
      Effect.gen(function* () {
        const tapped: Array<string> = []
        const result = yield* Effect.tapCauseIf(Effect.succeed(42), constTrue, () =>
          Effect.sync(() => tapped.push('tapped')),
        )
        assert.deepStrictEqual(tapped, [])
  ...
  describe('onErrorIf', () => {
    ...
    it.effect('predicate no match skips finalizer', () =>
      Effect.gen(function* () {
        const finalized: Array<string> = []
        const result = yield* Effect.exit(
          Effect.onErrorIf(Effect.fail('e1'), constFalse, () => Effect.sync(() => finalized.push('finalized'))),
        )
    ...
    it.effect('success skips finalizer', () =>
      Effect.gen(function* () {
        const finalized: Array<string> = []
        const result = yield* Effect.onErrorIf(Effect.succeed(42), constTrue, () =>
  ...
  describe('onExitIf', () => {
    it.effect('predicate match on success runs finalizer', () =>
      Effect.gen(function* () {
        const finalized: Array<string> = []
        const result = yield* Effect.onExitIf(Effect.succeed(42), Exit.isSuccess, (exit) => {
    ...
    it.effect('predicate no match on failure skips finalizer', () =>
      Effect.gen(function* () {
        const finalized: Array<string> = []
        const result = yield* Effect.exit(
          Effect.onExitIf(Effect.fail('e1'), Exit.isSuccess, () => Effect.sync(() => finalized.push('finalized'))),
        )
        assert.deepStrictEqual(finalized, [])
        assert.deepStrictEqual(result, Exit.fail('e1'))
      }),
  ...
  describe('filter with predicate/refinement', () => {
    it.effect('predicate filters iterable', () =>
      Effect.gen(function* () {
        const result = yield* Effect.filter([1, 2, 3, 4, 5], (n: number) => n % 2 === 0)
    ...
    it.effect('returns the first success and does not run later effects', () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.firstSuccessOf([
          Effect.sync(() => executed.push('first')).pipe(Effect.flatMap(() => Effect.fail('e1' as const))),
          Effect.sync(() => executed.push('second')).pipe(Effect.as('success' as const)),
          Effect.sync(() => executed.push('third')).pipe(Effect.as('unreachable' as const)),
        ])

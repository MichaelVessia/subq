...
describe('Option', () => {
  ...
  it('toString', () => {
    ...
    strictEqual(String(Option.some(Chunk.make(1, 2, 3))), `some(Chunk([1,2,3]))`)
  })
  ...
  it('toRefinement', () => {
    const f = (s: string | number): Option.Option<string> => (typeof s === 'string' ? Option.some(s) : Option.none())
  ...
  it('orElse', () => {
    const assertOrElse = (a: Option.Option<number>, b: Option.Option<number>, expected: Option.Option<number>) => {
      deepStrictEqual(
        pipe(
          a,
          Option.orElse(() => b),

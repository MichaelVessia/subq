...
describe('Layer', () => {
  it.effect('layers can be acquired in parallel', () =>
    Effect.gen(function* () {
      ...
      const layer2 = Layer.effectContext(
        Effect.acquireRelease(latch.open.pipe(Effect.map((bool) => Context.make(BoolTag, bool))), () => Effect.void),
      )
  ...
  it.effect('sharing itself with merge', () =>
    Effect.gen(function* () {
      ...
      const result = yield* env.pipe(Effect.map((context) => Context.get(context, Service1Tag)))
    ...
    it.effect('allows passing partial service', () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          yield* service.three.pipe(Stream.catchCause(Stream.fail), Stream.runDrain, Effect.flip)
        }).pipe(
          Effect.provide(
            Layer.mock(Service1)({

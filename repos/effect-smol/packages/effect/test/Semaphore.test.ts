...
describe('Semaphore', () => {
  ...
  it.effect('requesting more permits than total returns never', () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* Effect.forkChild(sem.withPermits('partition-1', 5)(Effect.succeed(42)))
  ...
  it.effect('interruption releases permits', () =>
    Effect.gen(function* () {
      ...
      yield* sem.withPermits('partition-1', 2)(Effect.sync(() => messages.push('after-interrupt')))
  ...
  it.effect('interruption with partial permit acquisition releases all taken permits', () =>
    Effect.gen(function* () {
      ...
      const fiber2 = yield* Effect.forkChild(
        sem.withPermits('partition-2', 3)(Effect.sync(() => messages.push('second'))),
      )

      yield* TestClock.adjust(Duration.millis(1))
      yield* Fiber.interrupt(fiber2)

      yield* sem.withPermits('partition-3', 1)(Effect.sync(() => messages.push('can-acquire-one')))
      ...
      yield* sem.withPermits('partition-4', 3)(Effect.sync(() => messages.push('all-three')))
  ...
  it.effect('take interruption does not leak permits', () =>
    Effect.gen(function* () {
      ...
      const waiter = yield* sem.take(1).pipe(Effect.provideService(Scheduler.Scheduler, scheduler), Effect.forkChild)
  ...
  it.effect('partial permit allocation', () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* Effect.forkChild(
        sem.withPermits('partition-2', 2)(Effect.sync(() => messages.push('second'))),
      )
  ...
  it.effect('sequential tasks in same partition', () =>
    Effect.gen(function* () {
      ...
      yield* sem.withPermits('partition-1', 2)(Effect.sync(() => messages.push('task-1')))

      yield* sem.withPermits('partition-1', 2)(Effect.sync(() => messages.push('task-2')))

      yield* sem.withPermits('partition-1', 2)(Effect.sync(() => messages.push('task-3')))

  ...
  describe('subscriptions', () => {
    ...
    it.effect('changesStream emits initial value then updates', () =>
      Effect.gen(function* () {
        ...
        const initial = yield* Stream.runCollect(TxSubscriptionRef.changesStream(ref).pipe(Stream.take(1)))
  ...
  describe('pipe support', () => {
    ...
    it.effect('modify data-last style', () =>
      Effect.tx(
        Effect.gen(function* () {
          const ref = yield* TxSubscriptionRef.make(10)
          const result = yield* ref.pipe(TxSubscriptionRef.modify((n: number) => [n * 2, n + 1]))

...
describe('Cause', () => {
  ...
  describe('fromReasons', () => {
    ...
    it('supports mixed reason types', () => {
      const reasons = [Cause.makeFailReason('error'), Cause.makeDieReason('defect'), Cause.makeInterruptReason(1)]
      const cause = Cause.fromReasons(reasons)
  ...
  describe('combine', () => {
    ...
    it('combines mixed reason types', () => {
      const combined = Cause.combine(Cause.fail('error'), Cause.combine(Cause.die('defect'), Cause.interrupt(1)))

...
const AuthedStream = Rpc.make('AuthedStream', { success: Schema.String, stream: true }).middleware(AuthMiddleware)
const AuthedGetUser = Rpc.make('AuthedGetUser', { success: Schema.String }).middleware(AuthMiddleware)
...
describe('Rpc', () => {
  ...
  describe('ExtractProvides', () => {
    ...
    it('extracts union of provides from multiple middlewares', () => {
      expect<Rpc.ExtractProvides<typeof MultiMiddleware, 'MultiMiddleware'>>().type.toBe<CurrentUser | DbConnection>()
    })

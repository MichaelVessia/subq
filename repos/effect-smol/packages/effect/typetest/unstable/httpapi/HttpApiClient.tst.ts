...
describe('HttpApiClient', () => {
  describe('path option', () => {
    it('should accept a record of fields', () => {
      ...
      const client = Effect.runSync(HttpApiClient.make(Api).pipe(Effect.provide(FetchHttpClient.layer)))
  ...
  describe('urlBuilder', () => {
    ...
    it('should support prefixes and top-level endpoints', () => {
      const Api = HttpApi.make('Api')
        ...
        .add(HttpApiGroup.make('top', { topLevel: true }).add(HttpApiEndpoint.get('topHealth', '/top-health')))
        .prefix('/v1')
  ...
  describe('payload option', () => {
    it('should default to void', () => {
      const Api = HttpApi.make('Api').add(HttpApiGroup.make('group').add(HttpApiEndpoint.get('a', '/a')))
      const client = Effect.runSync(HttpApiClient.make(Api).pipe(Effect.provide(FetchHttpClient.layer)))
      const f = client.group.a
  ...
  describe('success option', () => {
    it('should accept a schema', () => {
      ...
      expect(client.group.a()).type.toBe<
        Effect.Effect<{ readonly a: number }, HttpClientError.HttpClientError | Schema.SchemaError>
      >()
    })
  ...
  describe('error option', () => {
    it('should default to client and schema errors', () => {
      const Api = HttpApi.make('Api').add(HttpApiGroup.make('group').add(HttpApiEndpoint.get('a', '/a')))
      const client = Effect.runSync(HttpApiClient.make(Api).pipe(Effect.provide(FetchHttpClient.layer)))
      expect(client.group.a()).type.toBe<Effect.Effect<void, HttpClientError.HttpClientError | Schema.SchemaError>>()
    })
    ...
    it('should accept a schema', () => {
      ...
      const client = Effect.runSync(HttpApiClient.make(Api).pipe(Effect.provide(FetchHttpClient.layer)))
      expect(client.group.a()).type.toBe<
        Effect.Effect<void, { readonly a: number } | HttpClientError.HttpClientError | Schema.SchemaError>
      >()
    })
  })
  ...
  describe('client middleware', () => {
    it('requiredForClient requires layer and includes required client errors', () => {
      ...
      expect(client.group.a()).type.toBe<
        Effect.Effect<string, RequiredClientError | HttpClientError.HttpClientError | Schema.SchemaError>
      >()
    ...
    it('requiredForClient is enforced for makeWith, group, and endpoint', () => {
      ...
      const middlewareLayer = HttpApiMiddleware.layerClient(RequiredMiddleware, ({ next, request }) => next(request))

      const withClient = Effect.runSync(
        HttpApiClient.makeWith(Api, { httpClient: TestHttpClient }).pipe(Effect.provide(middlewareLayer)),
      )
      const fromClient = withClient.group.a

      const withGroup = Effect.runSync(
        HttpApiClient.group(Api, { group: 'group', httpClient: TestHttpClient }).pipe(Effect.provide(middlewareLayer)),
      )

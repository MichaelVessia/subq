...
describe('Schema', () => {
  ...
  describe('make', () => {
    ...
    describe('Struct', () => {
      ...
      it('refine field', () => {
        ...
        expect(schema.make).type.toBe<Make<{ readonly a: Option.Some<string> }, { readonly a: Option.Some<string> }>>()
      })
    ...
    describe('Tuple', () => {
      ...
      it('nested defaults (Struct)', () => {
        ...
        expect(schema.make).type.toBe<Make<readonly [{ readonly b?: number }?], readonly [{ readonly b: number }]>>()
      })
      ...
      it('nested defaults (Tuple)', () => {
        ...
        expect(schema.make).type.toBe<Make<readonly [(readonly [number?])?], readonly [readonly [number]]>>()
      })
    })
  ...
  describe('annotateEncoded', () => {
    it('non-transforming schema should return Rebuild', () => {
      const schema = Schema.String.pipe(Schema.annotateEncoded({ title: 'encoded' }))
      expect(schema).type.toBe<Schema.String>()
    })

    it('transforming schema should return Rebuild', () => {
      const schema = Schema.NumberFromString.pipe(Schema.annotateEncoded({ title: 'encoded' }))
    ...
    it('should constrain annotations to the Encoded type', () => {
      ...
      expect(Schema.annotateEncoded<Schema.NumberFromString>).type.toBeCallableWith({ examples: ['a'] })
      // number annotations should not be valid for a string-encoded schema
      expect(Schema.annotateEncoded<Schema.NumberFromString>).type.not.toBeCallableWith({ examples: [1] })
    })
  })
  ...
  it('instanceOf', () => {
    ...
    expect(schema.make).type.toBe<(input: MyError, options?: Schema.MakeOptions | undefined) => MyError>()
  })
  ...
  describe('Class APIs', () => {
    ...
    describe('TaggedClass', () => {
      it('Fields argument', () => {
        ...
        expect(revealClass(A)).type.toBe<Schema.Class<A, Schema.TaggedStruct<'A', { readonly a: Schema.String }>, A>>()

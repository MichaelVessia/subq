...
describe('Struct', () => {
  ...
  describe('Opaque', () => {
    ...
    it('Struct', () => {
      ...
      >()
      expect(schema.ast).type.toBe<SchemaAST.Objects>()
      expect(schema.make).type.toBe<(input: { readonly a: number }, options?: Schema.MakeOptions | undefined) => A>()
...
describe('StructWithRest', () => {
  ...
  it('Record', () => {
    expect(Struct.Record([], 'value' as const)).type.toBe<Record<never, 'value'>>()
    expect(Struct.Record(['a', 'b'], 'value' as const)).type.toBe<Record<'a' | 'b', 'value'>>()
  })
})

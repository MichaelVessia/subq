...
declare const layerStringOrNumber: Layer.Layer<'out-1', 'err-1', 'dep-1'> | Layer.Layer<'out-2', 'err-2', 'dep-2'>
...
describe('Effect.forkScoped', () => {
  ...
  it('adds Scope to requirements in data-last usage', () => {
    const result = pipe(string, Effect.forkScoped(), Effect.flatMap(Fiber.join))
    expect(result).type.toBe<Effect.Effect<string, 'err-1', 'dep-1' | Scope.Scope>>()
  })
})
...
describe('Effect.acquireRelease', () => {
  it('supports dependencies in the release finalizer', () => {
    const result = Effect.acquireRelease(Effect.succeed('resource'), () => Effect.service(AcquireReleaseDependency))
...
describe('all', () => {
  ...
  it('struct', () => {
    ...
    expect(Effect.all({ a: string, b: number }, { mode: 'result' })).type.toBe<
      Effect.Effect<{ a: Result.Result<string, 'err-1'>; b: Result.Result<number, 'err-2'> }, never, 'dep-1' | 'dep-2'>
    >()
  ...
  it('array', () => {
    expect(Effect.all(stringArray)).type.toBe<Effect.Effect<Array<string>, 'err-3', 'dep-3'>>()
    expect(Effect.all(stringArray, undefined)).type.toBe<Effect.Effect<Array<string>, 'err-3', 'dep-3'>>()
    expect(Effect.all(stringArray, {})).type.toBe<Effect.Effect<Array<string>, 'err-3', 'dep-3'>>()
    expect(Effect.all(stringArray, { concurrency: 'unbounded' })).type.toBe<
      Effect.Effect<Array<string>, 'err-3', 'dep-3'>
    >()
    expect(Effect.all(stringArray, { discard: true })).type.toBe<Effect.Effect<void, 'err-3', 'dep-3'>>()
    ...
    >()
    expect(Effect.all(stringArray, { mode: 'result', discard: true })).type.toBe<Effect.Effect<void, never, 'dep-3'>>()
  })

  it('record', () => {
    expect(Effect.all(numberRecord)).type.toBe<Effect.Effect<{ [x: string]: number }, 'err-4', 'dep-4'>>()
    expect(Effect.all(numberRecord, undefined)).type.toBe<Effect.Effect<{ [x: string]: number }, 'err-4', 'dep-4'>>()
    expect(Effect.all(numberRecord, {})).type.toBe<Effect.Effect<{ [x: string]: number }, 'err-4', 'dep-4'>>()
    expect(Effect.all(numberRecord, { concurrency: 'unbounded' })).type.toBe<
      Effect.Effect<{ [x: string]: number }, 'err-4', 'dep-4'>
    >()
    expect(Effect.all(numberRecord, { discard: true })).type.toBe<Effect.Effect<void, 'err-4', 'dep-4'>>()
    ...
    >()
    expect(Effect.all(numberRecord, { mode: 'result', discard: true })).type.toBe<Effect.Effect<void, never, 'dep-4'>>()
  })
})

...
export const drain: Sink<void, unknown> = fromTransform((upstream) =>
  Pull.catchDone(Effect.forever(upstream, { disableYield: true }), () => endVoid),
)
 ...
 * @category constructors
 */
export const takeWhileFilter = <In, Out, X>(filter: Filter.Filter<In, Out, X>): Sink<Array<Out>, In, In> =>
  fromTransform((upstream) => {
...
export const provideContext: {
  ...
  <A, In, L, E, R, Provided>(
    ...
    fromTransform((upstream, scope) => self.transform(upstream, scope).pipe(Effect.provideContext(context))),
)

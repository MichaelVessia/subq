...
} = dual(2, <A>(self: TxSubscriptionRef<A>, value: A): Effect.Effect<void> => modify(self, () => [void 0, value]))
...
} = dual(2, <A>(self: TxSubscriptionRef<A>, value: A): Effect.Effect<A> => modify(self, (current) => [current, value]))
 ...
 */
export const changes = <A>(self: TxSubscriptionRef<A>): Effect.Effect<TxQueue.TxQueue<A>, never, Scope.Scope> =>
...
export const changesStream = <A>(self: TxSubscriptionRef<A>): Stream.Stream<A, never, never> =>
  Stream.unwrap(Effect.map(changes(self), (sub) => Stream.fromEffectRepeat(Effect.tx(TxQueue.take(sub)))))

 ...
 */
export const make = <Input, Pass, Fail>(f: (input: Input) => Result.Result<Pass, Fail>): Filter<Input, Pass, Fail> =>
  f as any
...
export const mapFail: {
  ...
  2,
  <Input, Pass, Fail, Fail2>(self: Filter<Input, Pass, Fail>, f: (value: Fail) => Fail2): Filter<Input, Pass, Fail2> =>
...
export const has =
  <K>(key: K) =>
  <Input extends { readonly has: (key: K) => boolean }>(input: Input): Result.Result<Input, Input> =>

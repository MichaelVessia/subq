...
export const htmlStream = <A extends ReadonlyArray<Template.InterpolatedWithStream>>(
  ...
  Effect.map(Effect.context<Template.Interpolated.Context<A[number]>>(), (context) =>
    stream(Stream.provideContext(Stream.encodeText(Template.stream(strings, ...args)), context), {
...
export const setCookie: {
  ...
  (
    ...
    Effect.map(Effect.fromResult(Cookies.set(self.cookies, name, value, options)), (cookies) =>
      makeResponse({
        ...self,
        cookies,
      }),
...
export const expireCookie: {
  ...
  (
    ...
    Effect.map(Effect.fromResult(Cookies.expireCookie(self.cookies, name, options)), (cookies) =>
      makeResponse({
        ...self,
        cookies,
      }),
...
export const toClientResponse = (
  ...
  new ServerHttpClientResponse(options?.request ?? HttpClientRequest.empty, response)
...
class ServerHttpClientResponse extends Inspectable.Class implements HttpClientResponse.HttpClientResponse, Pipeable {
  ...
  constructor(request: HttpClientRequest.HttpClientRequest, response: HttpServerResponse) {
    super()
 ...
 * @category conversions
 */
export const fromClientResponse = (response: HttpClientResponse.HttpClientResponse): HttpServerResponse => {
...
const isEmptyBodyError = (error: HttpClientError.HttpClientError): error is HttpClientError.HttpClientError =>

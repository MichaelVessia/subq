  ...
  <E>(
    ...
    const httpApp = toHandled(
      ...
      (request, response) => {
        ...
        ;(request as any)[resolveSymbol](Response.toWeb(response, { withoutBody: request.method === 'HEAD', context }))
        return Effect.void
      },
      middleware,
    )
...
>(
  ...
  let handlerPromise:
    ...
    | undefined
  function handler(request: Request, context?: Context.Context<ReqR> | undefined): Promise<globalThis.Response> {
    if (handlerCache) {

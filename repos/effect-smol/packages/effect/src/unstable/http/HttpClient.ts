...
export const withRateLimiter: {
  ...
  <E, R>(
    ...
    const resolveTokens: (request: HttpClientRequest.HttpClientRequest) => number =
      typeof tokensOption === 'function' ? tokensOption : constant(tokensOption ?? 1)
...
const parseRateLimitLimit = (state: RateLimiterState, headers: Headers.Headers, tokens: number): number | undefined => {
...
const parseRateLimitWindow = (clock: Clock, headers: Headers.Headers): Duration.Duration | undefined => {
...
const parseRetryAfter = (clock: Clock, value: string | undefined): Duration.Duration | undefined => {
  if (value === undefined) {
    return undefined
  }
...
const parseResetHeader = (clock: Clock, value: string | undefined): Duration.Duration | undefined => {
  const numeric = parseNumberHeader(value)
  if (numeric === undefined || numeric <= 0) {
    return undefined
  }
 ...
 * @category References
 */
export const TracerDisabledWhen = Context.Reference<Predicate.Predicate<HttpClientRequest.HttpClientRequest>>(
 ...
 * @category References
 */
export const SpanNameGenerator = Context.Reference<(request: HttpClientRequest.HttpClientRequest) => string>(
 ...
 * @since 4.0.0
 */
export const layerMergedContext = <E, R>(effect: Effect.Effect<HttpClient, E, R>): Layer.Layer<HttpClient, E, R> =>

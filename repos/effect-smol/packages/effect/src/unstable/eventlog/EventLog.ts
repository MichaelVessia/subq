...
export class Registry extends Context.Service<
  Registry,
  {
    readonly registerHandlerUnsafe: (options: { readonly event: string; readonly handler: Handlers.Item<any> }) => void
  ...
  const runRemote = Effect.fnUntraced(
    function* (remote: EventLogRemote['Service']) {
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.scoped,
        Effect.catchCause(Effect.logError),
        Effect.repeat(Schedule.exponential(200, 1.5).pipe(Schedule.either(Schedule.spaced({ seconds: 10 })))),
        Effect.annotateLogs({
          service: 'EventLog',
...
export const layerEventLog: Layer.Layer<EventLog | Registry, never, EventJournal | Identity> = Layer.effect(
  EventLog,
  make,
).pipe(Layer.provide(ReactivityLayer.layer), Layer.provideMerge(layerRegistry))

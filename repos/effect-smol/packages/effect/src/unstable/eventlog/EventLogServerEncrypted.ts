  ...
  Effect.gen(function* () {
    ...
    return EventLogServer.layerRpcHandlers({
      ...
      changes: ({ publicKey, storeId, startSequence }) =>
        storage
          .changes(publicKey, storeId, startSequence)
          .pipe(Stream.mapArrayEffect((entries) => Effect.map(ChangesRpc.encodeEncrypted(entries), Arr.of))),
    })
  }),
)
...
export const makeStorageMemory: Effect.Effect<Storage['Service'], never, Scope.Scope> = Effect.gen(function* () {
  ...
  const pubsubs = yield* RcMap.make({
    lookup: (_scopeKey: string) => Effect.acquireRelease(PubSub.unbounded<EncryptedRemoteEntry>(), PubSub.shutdown),

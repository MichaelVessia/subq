  ...
  const retryForbidden = <A, E, R>(
    ...
    Effect.retry(effect, {
      while(e) {
        hello = null
        const isForbidden =
          Predicate.isTagged(e, 'EventLogProtocolError') && (e as any as EventLogProtocolError).code === 'Forbidden'
...
> {
  ...
  return yield* makeWith({
    ...
    decodeChanges: (identity, data) =>
      ChangesRpc.decodeEncrypted(data).pipe(Effect.flatMap((entries) => encryption.decrypt(identity, entries))),
  })
})
 ...
 * @category Layers
 */
export const layerEncrypted: Layer.Layer<EventLogRemote, EventLogRemoteError, RpcClient.Protocol | Registry> =
 ...
 * @category Layers
 */
export const layerUnencrypted: Layer.Layer<EventLogRemote, EventLogRemoteError, RpcClient.Protocol | Registry> =
  Layer.effect(EventLogRemote, makeUnencrypted).pipe(Layer.provide(EventLogRemoteClient.layer))

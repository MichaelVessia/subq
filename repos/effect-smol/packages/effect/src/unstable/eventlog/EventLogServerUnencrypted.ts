...
const toStoreNotFoundError = (options: { readonly storeId: StoreId; readonly publicKey?: string | undefined }) =>
 ...
 * @category store
 */
export const layerStoreMappingStatic = (options: { readonly storeId: StoreId }): Layer.Layer<StoreMapping> =>
...
export const makeStorageMemory: Effect.Effect<Storage['Service'], never, Scope.Scope> = Effect.gen(function* () {
  ...
  const pubsubs = yield* RcMap.make({
    lookup: (_storeId: string) => Effect.acquireRelease(PubSub.unbounded<RemoteEntry>(), PubSub.shutdown),
  ...
  return Storage.of({
    ...
    changes: Effect.fnUntraced(function* ({ storeId, startSequence, compactors }) {
      ...
      return Stream.fromArray(backlog).pipe(
        Stream.concat(
          Stream.fromSubscription(subscription).pipe(Stream.filter((entry) => entry.remoteSequence > replayedUpTo)),
        ),
      )
    }, Stream.unwrap),
 ...
 * @category layers
 */
export const layerServer: Layer.Layer<EventLogServerUnencrypted | EventLog.Registry, never, Storage> = Layer.effect(
  EventLogServerUnencrypted,
  make,
).pipe(Layer.provideMerge(EventLog.layerRegistry))

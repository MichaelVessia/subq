      ...
      changes: Effect.fnUntraced(
        function* ({ storeId, startSequence, compactors }) {
          ...
          return Stream.fromArray(backlog).pipe(
            Stream.concat(
              Stream.fromSubscription(subscription).pipe(Stream.filter((entry) => entry.remoteSequence > watermark)),
            ),
          )
        },
        Effect.orDie,
...
const decodeRemoteEntries = (rows: unknown): Effect.Effect<Array<RemoteEntry>, Schema.SchemaError> =>
  decodeRemoteEntryRows(rows).pipe(Effect.map((rows) => rows.map(toRemoteEntry)))

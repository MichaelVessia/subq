...
> => {
  ...
  >({
    ...
    resolver: Effect.fnUntraced(function* (entries) {
      ...
      const results = yield* options.execute(inputs as any).pipe(Effect.provideContext(entries[0].context))
...
> =>
  ...
  >({
    ...
    resolver: Effect.fnUntraced(function* (entries) {
      const inputs = yield* partitionRequests(entries, options.Request)
      yield* options.execute(inputs as any).pipe(Effect.provideContext(entries[0].context))
      for (let i = 0; i < entries.length; i++) {

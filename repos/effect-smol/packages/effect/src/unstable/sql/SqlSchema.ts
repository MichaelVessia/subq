 ...
 */
export const findAll = <Req extends Schema.Top, Res extends Schema.Top, E, R>(options: {
 ...
 * @category constructor
 */
export const findNonEmpty = <Req extends Schema.Top, Res extends Schema.Top, E, R>(options: {
  readonly Request: Req
  readonly Result: Res
  readonly execute: (request: Req['Encoded']) => Effect.Effect<ReadonlyArray<unknown>, E, R>
}) => {
  ...
  > =>
    Effect.flatMap(find(request), (results) =>
      Arr.isArrayNonEmpty(results) ? Effect.succeed(results) : Effect.fail(new Cause.NoSuchElementError()),
    )
}
 ...
 * @category constructor
 */
export const findOneOption = <Req extends Schema.Top, Res extends Schema.Top, E, R>(options: {
  readonly Request: Req
  readonly Result: Res
  readonly execute: (request: Req['Encoded']) => Effect.Effect<ReadonlyArray<unknown>, E, R>
}) => {

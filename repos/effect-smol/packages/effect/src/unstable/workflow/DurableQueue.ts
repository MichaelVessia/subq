...
>(options: {
  ...
  [TypeId]: TypeId,
  name: options.name,
  payloadSchema: Schema.isSchema(options.payload) ? options.payload : (Schema.Struct(options.payload as any) as any),
 ...
 * @category Processing
 */
export const process: <Payload extends Schema.Top, Success extends Schema.Top, Error extends Schema.Top>(
...
> = Effect.fnUntraced(function* <Payload extends Schema.Top, Success extends Schema.Top, Error extends Schema.Top>(
  self: DurableQueue<Payload, Success, Error>,
...
const defaultRetrySchedule = Schedule.exponential(500, 1.5).pipe(Schedule.either(Schedule.spaced('1 minute')))
 ...
 * @category Worker
 */
export const makeWorker: <Payload extends Schema.Top, Success extends Schema.Top, Error extends Schema.Top, R>(
  self: DurableQueue<Payload, Success, Error>,
...
> = Effect.fnUntraced(function* <Payload extends Schema.Top, Success extends Schema.Top, Error extends Schema.Top, R>(
  self: DurableQueue<Payload, Success, Error>,
  f: (payload: Payload['Type']) => Effect.Effect<Success['Type'], Error['Type'], R>,
  options?: {
 ...
 * @category Worker
 */
export const worker: <Payload extends Schema.Top, Success extends Schema.Top, Error extends Schema.Top, R>(
  self: DurableQueue<Payload, Success, Error>,
  f: (payload: Payload['Type']) => Effect.Effect<Success['Type'], Error['Type'], R>,
  options?:
    | {

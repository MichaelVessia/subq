...
export const makeNoSerialization: <Rpcs extends Rpc.Any>(
  ...
  Effect.fnUntraced(function* <Rpcs extends Rpc.Any>(
    ...
    const handleRequest = (
      ...
      if (enableTracing) {
        const parentSpan = requestFiber.context.mapUnsafe.get(Tracer.ParentSpan.key) as Tracer.AnySpan | undefined
      ...
      if (!schemas) {
        ...
        schemas = {
          ...
          encodeChunk: Schema.encodeUnknownEffect(
            Schema.toCodecJson(Schema.Array(Option.isSome(streamSchemas) ? streamSchemas.value.success : Schema.Any)),
          ) as any,

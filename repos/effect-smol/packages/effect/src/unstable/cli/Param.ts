...
} = dual(2, <Kind extends ParamKind, A, B>(self: Param<Kind, A>, prompt: FallbackPrompt<B>): Param<Kind, A | B> => {
      ...
      (parse: Parse<A>): Parse<Result.Result<A, B>> =>
        (args: ParsedArgs) =>
          Effect.catch(
            ...
            (err) =>
              Effect.map(orElse(err).parse(args), ([leftover, value]) => [leftover, Result.fail(value)] as const),
          ),
    )
  },
)

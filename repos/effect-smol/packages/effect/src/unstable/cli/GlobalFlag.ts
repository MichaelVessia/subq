...
export const setting =
  ...
  <A>(options: { readonly flag: Flag.Flag<A> }): Setting<Id, A> => {
    settingIdCounter += 1
    const ref = Context.Service<Setting.Identifier<Id>, A>(`effect/unstable/cli/GlobalFlag/${id}/${settingIdCounter}`)
...
export const Completions: Action<Option.Option<'bash' | 'zsh' | 'fish'>> = action({
  flag: Flag.choice('completions', ['bash', 'zsh', 'fish', 'sh'] as const).pipe(
    Flag.optional,
 ...
 * @category references
 */
export const BuiltIns: ReadonlyArray<GlobalFlag<any>> = [Help, Version, Completions, LogLevel]

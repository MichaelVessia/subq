...
const handleConfirmClear = (options: ConfirmOptionsReq) => {
  return Effect.fnUntraced(function* (state: ConfirmState, _: Action<ConfirmState, boolean>) {
    ...
    const promptText = renderConfirmOutput(confirmMessage, '?', figures.pointerSmall, options, { plain: true })
...
const renderPrompt = (
  ...
  const annotate = options?.plain === true ? (line: string) => line : annotateLine
...
const renderMultiSelectTitle = (title: string, isHighlighted: boolean, renderOptions?: RenderOptions | undefined) => {
  ...
  for (let index = toDisplay.startIndex; index < toDisplay.endIndex; index++) {
    ...
    if (index < metaOptions.length) {
      ...
      const annotatedCheckbox =
        isHighlighted && renderOptions?.plain !== true ? Ansi.annotate(checkbox, Ansi.cyanBright) : checkbox
      const title = renderMultiSelectTitle(choice.title, isHighlighted, renderOptions)
...
const renderNumberError = (state: NumberState, pointer: string, renderOptions?: RenderOptions | undefined) => {
  if (Option.isSome(state.error)) {
    return Arr.match(state.error.value.split(NEWLINE_REGEXP), {
      onEmpty: () => '',
      onNonEmpty: (errorLines) => {
...
const renderTextError = (nextState: TextState, pointer: string, renderOptions?: RenderOptions | undefined): string => {

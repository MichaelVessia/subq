...
export const makeStoreRedis = Effect.fnUntraced(function* (options?: {
  ...
  const lockRefreshMillis = Math.max(
    options?.lockRefreshInterval ? Duration.toMillis(Duration.fromInputUnsafe(options.lockRefreshInterval)) : 30_000,
    1,
  )
  const lockExpirationMillis = Math.max(
    options?.lockExpiration ? Duration.toMillis(Duration.fromInputUnsafe(options.lockExpiration)) : 90_000,
    1,
  )
...
export const layerStoreRedis: (
  options?:
    ...
    | undefined,
) => Layer.Layer<PersistedQueueStore, never, Redis.Redis> = flow(makeStoreRedis, Layer.effect(PersistedQueueStore))

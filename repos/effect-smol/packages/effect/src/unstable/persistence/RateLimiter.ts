 ...
 * @category Errors
 */
export const RateLimiterErrorReason: Schema.Union<[typeof RateLimitExceeded, typeof RateLimitStoreError]> =
...
}) => Layer.Layer<RateLimiterStore, never, Redis.Redis> = flow(makeStoreRedis, Layer.effect(RateLimiterStore))
...
export const layerStoreRedisConfig = (
  ...
  Layer.effect(RateLimiterStore, Effect.flatMap(Config.unwrap(options), makeStoreRedis))

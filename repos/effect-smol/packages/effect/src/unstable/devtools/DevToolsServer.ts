...
) => Effect.Effect<never, SocketServer.SocketServerError, R | SocketServer.SocketServer> = Effect.fnUntraced(function* <
  R,
  E,
  _,
>(handle: (client: Client) => Effect.Effect<_, E, R>) {
    ...
    Effect.fnUntraced(function* (socket) {
      ...
      yield* Stream.fromQueue(responses).pipe(
        ...
        Stream.runForEach((request) =>
          request._tag === 'Ping' ? Queue.offer(responses, { _tag: 'Pong' }) : Queue.offer(requests, request),
        ),

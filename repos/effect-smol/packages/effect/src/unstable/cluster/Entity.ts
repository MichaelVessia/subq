...
const Proto = {
  ...
  get client() {
    return shardingTag.pipe(Effect.flatMap((sharding) => sharding.makeClient(this as any)))
  },
 ...
 * @category context
 */
export class CurrentAddress extends Context.Service<CurrentAddress, EntityAddress>()(
 ...
 * @category context
 */
export class CurrentRunnerAddress extends Context.Service<CurrentRunnerAddress, RunnerAddress>()(
...
> = Effect.fnUntraced(function* <Type extends string, Rpcs extends Rpc.Any, LA, LE, LR>(
  ...
  const entityMap = new Map<
    string,
    {
      readonly context: Context.Context<Rpc.ServicesClient<Rpcs> | Rpc.ServicesServer<Rpcs> | Rpc.Middleware<Rpcs> | LR>
      readonly concurrency: number | 'unbounded'
      readonly build: Effect.Effect<Context.Context<Rpc.ToHandler<Rpcs>>, never, Scope | CurrentAddress>
    }
  >()
            ...
            .pipe(
              Effect.provideContext(
                Context.mutate(context, (context) =>
                  context.pipe(Context.add(CurrentRunnerAddress, runnerAddress), Context.omit(Scope)),
                ),
              ),
            ) as any,
        })

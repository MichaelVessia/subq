...
type UrlBuilderRequest<Endpoint extends HttpApiEndpoint.Any> = ([HttpApiEndpoint.Params<Endpoint>['Type']] extends [
  never,
]
  ? {}
  ...
  ([HttpApiEndpoint.Query<Endpoint>['Type']] extends [never]
    ? {}
    : { readonly query: HttpApiEndpoint.Query<Endpoint>['Type'] }) extends infer Request
...
export const urlBuilder = <Api extends HttpApi.Any>(
  ...
  HttpApi.reflect(api as unknown as HttpApi.AnyWithProps, {
    ...
    onEndpoint({ group, endpoint }) {
      const makeUrl = compilePath(endpoint.path)
      const encodeParams =
        endpoint.params === undefined ? undefined : Schema.encodeSync(endpoint.params as Schema.Encoder<unknown>)
      const encodeQuery =
        endpoint.query === undefined ? undefined : Schema.encodeSync(endpoint.query as Schema.Encoder<unknown>)

      const endpointBuilder = (request?: { readonly params?: unknown; readonly query?: unknown }) => {
...
function getEncodePayloadSchemaFromBody(schema: Schema.Top, method: HttpMethod.HttpMethod): Schema.Top {
      ...
      Transformation.transformOrFail<unknown, HttpBody.HttpBody>({
        ...
        encode(t) {
          switch (encoding._tag) {
            ...
            case 'Text': {
              if (typeof t !== 'string') {
                return Effect.fail(new Issue.InvalidValue(Option.some(t), { message: 'Expected a string' }))
              }
            ...
            case 'Uint8Array': {
              if (!(t instanceof Uint8Array)) {
                return Effect.fail(new Issue.InvalidValue(Option.some(t), { message: 'Expected a Uint8Array' }))
              }

...
> =>
  Layer.effectContext(
    Effect.gen(function* () {
      const services = (yield* Effect.context<any>()).pipe(Context.omit(Scope.Scope))
...
> =>
  Effect.contextWith((context: Context.Context<any>) => {
    ...
    return Effect.succeed(
      handlerToHttpEffect(group, endpoint, Context.omit(Scope.Scope)(context), handler as any, false),
    )
  })
...
function buildPayloadDecoders(payloadMap: HttpApiEndpoint.PayloadMap): Map<string, PayloadDecoder> {
...
const getRequestContentType = (request: HttpServerRequest): string =>
  request.headers['content-type'] ? request.headers['content-type'].toLowerCase().trim() : 'application/json'

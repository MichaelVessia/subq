...
export const manual = <A, E, R>(
  ...
  Effect.contextWith((context: Context.Context<R>) => {
    const providedAcquire = Effect.updateContext(acquire, (input: Context.Context<never>) =>
      Context.merge(context, input),
    )
    return Effect.map(ScopedRef.fromAcquire(Effect.exit(providedAcquire)), (scopedRef) =>
      makeUnsafe(scopedRef, providedAcquire),
    )
  })
...
export const auto = <A, E, R, Out, E2, R2>(
  ...
  Effect.tap(manual(acquire), (self) => Effect.forkScoped(Effect.repeat(refresh(self), policy)))

...
export const filterMap = dual<
  <A, B, X>(f: (input: A, key: string) => Result.Result<B, X>) => (self: TR.Trie<A>) => TR.Trie<B>,
...
export const compact = <A>(self: TR.Trie<Option.Option<A>>) =>
  reduce(self, empty<A>(), (trie, option, key) => (Option.isSome(option) ? insert(trie, key, option.value) : trie))

...
export abstract class ReasonBase<Tag extends string> implements Cause.Cause.ReasonProto<Tag> {
  ...
  annotate(annotations: Context.Context<never>, options?: { readonly overwrite?: boolean | undefined }): this {
...
export class Fail<E> extends ReasonBase<'Fail'> implements Cause.Fail<E> {
  ...
  [Equal.symbol](that: any): boolean {
    return (
      isFailReason(that) && Equal.equals(this.error, that.error) && Equal.equals(this.annotations, that.annotations)
    )
  }
  [Hash.symbol](): number {
...
/** @internal */
export const causeFromReasons = <E>(reasons: ReadonlyArray<Cause.Reason<E>>): Cause.Cause<E> => new CauseImpl(reasons)
...
export class Die extends ReasonBase<'Die'> implements Cause.Die {
  ...
  [Equal.symbol](that: any): boolean {
    return (
      isDieReason(that) && Equal.equals(this.defect, that.defect) && Equal.equals(this.annotations, that.annotations)
    )
  }
  [Hash.symbol](): number {
...
/** @internal */
export const isFailReason = <E>(self: Cause.Reason<E>): self is Cause.Fail<E> => self._tag === 'Fail'

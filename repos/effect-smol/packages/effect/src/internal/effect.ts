...
/** @internal */
export const causeFilterInterruptors = <E>(self: Cause.Cause<E>): Result.Result<Set<number>, Cause.Cause<E>> => {
...
/** @internal */
export const reasonAnnotations = <E>(self: Cause.Reason<E>): Context.Context<never> =>
  Context.makeUnsafe(self.annotations)

/** @internal */
export const causeAnnotations = <E>(self: Cause.Cause<E>): Context.Context<never> => {
...
export const causeCombine: {
  ...
  const newCause = new CauseImpl<E | E2>(Arr.union(self.reasons, that.reasons))
...
export class FiberImpl<A = any, E = any> implements Fiber.Fiber<A, E> {
  ...
  runLoop(effect: Primitive): Exit.Exit<A, E> | Yield {
    ...
    try {
      while (true) {
        this.currentOpCount++
        if (!yielding && !this.currentPreventYield && this.currentScheduler.shouldYield(this as any)) {
          yielding = true
          const prev = current
...
> => suspend(() => fromIteratorUnsafe(args.length === 1 ? args[0]() : (args[1].call(args[0].self) as any)))
...
/** @internal */
export const fnUntracedEager: Effect.fn.Untraced = (body: Function, ...pipeables: Array<any>) =>
...
export const andThen: {
  ...
  <B, E2, R2>(f: Effect.Effect<B, E2, R2>): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<B, E | E2, R | R2>
  <A, E, R, B, E2, R2>(
    self: Effect.Effect<A, E, R>,
    f: (a: A) => Effect.Effect<B, E2, R2>,
  ): Effect.Effect<B, E | E2, R | R2>
  <A, E, R, B, E2, R2>(self: Effect.Effect<A, E, R>, f: Effect.Effect<B, E2, R2>): Effect.Effect<B, E | E2, R | R2>
} = dual(
  2,
  <A, E, R, B, E2, R2>(
    self: Effect.Effect<A, E, R>,
...
export const tap: {
  ...
  <B, E2, R2>(f: Effect.Effect<B, E2, R2>): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<A, E | E2, R | R2>
  <A, E, R, B, E2, R2>(
    self: Effect.Effect<A, E, R>,
    f: (a: NoInfer<A>) => Effect.Effect<B, E2, R2>,
  ): Effect.Effect<A, E | E2, R | R2>
  <A, E, R, B, E2, R2>(self: Effect.Effect<A, E, R>, f: Effect.Effect<B, E2, R2>): Effect.Effect<A, E | E2, R | R2>
} = dual(
  2,
  <A, E, R, B, E2, R2>(
    self: Effect.Effect<A, E, R>,
...
/** @internal */
export const exitFilterValue = <A, E>(self: Exit.Exit<A, E>): Result.Result<A, Exit.Failure<never, E>> =>
...
/** @internal */
export const exitFilterCause = <A, E>(self: Exit.Exit<A, E>): Result.Result<Cause.Cause<E>, Exit.Success<A>> =>
  self._tag === 'Failure' ? Result.succeed(self.cause) : Result.fail(self as any)

/** @internal */
export const exitFindError = Filter.composePassthrough(exitFilterCause, findError)

/** @internal */
export const exitFindDefect = Filter.composePassthrough(exitFilterCause, findDefect)

/** @internal */
export const exitHasInterrupts = <A, E>(self: Exit.Exit<A, E>): self is Exit.Failure<A, E> =>
  self._tag === 'Failure' && hasInterrupts(self.cause)

/** @internal */
export const exitHasDies = <A, E>(self: Exit.Exit<A, E>): self is Exit.Failure<A, E> =>
  self._tag === 'Failure' && hasDies(self.cause)

/** @internal */
export const exitHasFails = <A, E>(self: Exit.Exit<A, E>): self is Exit.Failure<A, E> =>
...
/** @internal */
export const serviceOption = <I, S>(service: Context.Key<I, S>): Effect.Effect<Option.Option<S>> =>
  withFiber((fiber) => succeed(Context.getOption(fiber.context, service)))

/** @internal */
export const serviceOptional = <I, S>(service: Context.Key<I, S>): Effect.Effect<S, Cause.NoSuchElementError> =>
  withFiber((fiber) =>
...
export const provideContext: {
  <XR>(context: Context.Context<XR>): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<A, E, Exclude<R, XR>>
  <A, E, R, XR>(self: Effect.Effect<A, E, R>, context: Context.Context<XR>): Effect.Effect<A, E, Exclude<R, XR>>
} = dual(
  2,
  <A, E, R, XR>(self: Effect.Effect<A, E, R>, context: Context.Context<XR>): Effect.Effect<A, E, Exclude<R, XR>> => {
...
export const catchNoSuchElement = <A, E, R>(
  ...
  matchEffect(self, {
    onFailure: (error) =>
      isNoSuchElementError(error) ? succeedNone : fail(error as Exclude<E, Cause.NoSuchElementError>),
...
export const catchTag: {
  ...
  >(
    ...
    orElse?:
      ...
      | undefined,
  ): <A, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<A | A1 | A2, E1 | E2, R | R1 | R2>
  <
    A,
    E,
    R,
...
export const catchTags: {
  ...
  (args) => isEffect(args[0]),
  (self: Effect.Effect<any, any, any>, cases: Record<string, any>, orElse: any) => {
...
/** @internal */
export const delay: {
  (duration: Duration.Input): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<A, E, R>
  <A, E, R>(self: Effect.Effect<A, E, R>, duration: Duration.Input): Effect.Effect<A, E, R>
} = dual(
  2,
  <A, E, R>(self: Effect.Effect<A, E, R>, duration: Duration.Input): Effect.Effect<A, E, R> =>
...
export const timeoutOrElse: {
  ...
  ): Effect.Effect<A | A2, E | E2, R | R2> => raceFirst(self, flatMap(sleep(options.duration), options.orElse)),
)

/** @internal */
export const timeout: {
  (duration: Duration.Input): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<A, E | Cause.TimeoutError, R>
  <A, E, R>(self: Effect.Effect<A, E, R>, duration: Duration.Input): Effect.Effect<A, E | Cause.TimeoutError, R>
} = dual(
  2,
  <A, E, R>(self: Effect.Effect<A, E, R>, duration: Duration.Input): Effect.Effect<A, E | TimeoutError, R> =>
    timeoutOrElse(self, {
      duration,
...
export const timeoutOption: {
  (duration: Duration.Input): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<Option.Option<A>, E, R>
  <A, E, R>(self: Effect.Effect<A, E, R>, duration: Duration.Input): Effect.Effect<Option.Option<A>, E, R>
} = dual(
  2,
  <A, E, R>(self: Effect.Effect<A, E, R>, duration: Duration.Input): Effect.Effect<Option.Option<A>, E, R> =>
    ...
    uninterruptibleMask((restore) =>
      flatMap(scope, (scope) =>
        tap(options?.interruptible ? restore(acquire) : acquire, (a) =>
          scopeAddFinalizerExit(scope, (exit) => provideContext(release(a, exit), context)),
        ),
      ),
    ),
  )
...
export const acquireUseRelease = <Resource, E, R, A, E2, R2, E3, R3>(
  ...
  uninterruptibleMask((restore) =>
    flatMap(acquire, (a) => onExitPrimitive(restore(use(a)), (exit) => release(a, exit), true)),
  )
...
export const cachedWithTTL: {
  (timeToLive: Duration.Input): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<Effect.Effect<A, E, R>>
  <A, E, R>(self: Effect.Effect<A, E, R>, timeToLive: Duration.Input): Effect.Effect<Effect.Effect<A, E, R>>
} = dual(
  2,
  <A, E, R>(self: Effect.Effect<A, E, R>, timeToLive: Duration.Input): Effect.Effect<Effect.Effect<A, E, R>> =>
...
export const validate: {
  ...
  <A, B, E, R>(
    ...
    flatMap(partition(elements, f, { concurrency: options?.concurrency }), ([excluded, satisfying]) => {
        ...
        if (effectIsExit(eff)) {
          ...
          fiber.addObserver((exit) => {
            ...
            if (terminal) {
              ...
              if (result) {
                terminal =
                  result._tag === 'Failure' ? exitFailCause(causeFromReasons(result.cause.reasons.slice())) : result
                go()
              }
            }
...
export const filterMapOrElse: {
  ...
  <A, E, R, B, X, C, E2, R2>(
    ...
    flatMap(self, (a) => {
      ...
      return (Result.isFailure(result) ? orElse(result.failure) : succeed(result.success)) as Effect.Effect<
        B | C,
        E2,
        R2
      >
...
/** @internal */
export const filter: {
  <A, B extends A>(refinement: Predicate.Refinement<NoInfer<A>, B>): (elements: Iterable<A>) => Effect.Effect<Array<B>>
  <A>(predicate: Predicate.Predicate<NoInfer<A>>): (elements: Iterable<A>) => Effect.Effect<Array<A>>
  <A, E, R>(
    predicate: (a: NoInfer<A>, i: number) => Effect.Effect<boolean, E, R>,
    options?: { readonly concurrency?: Concurrency | undefined },
  ): (iterable: Iterable<A>) => Effect.Effect<Array<A>, E, R>
  <A, B extends A>(elements: Iterable<A>, refinement: Predicate.Refinement<A, B>): Effect.Effect<Array<B>>
  <A>(elements: Iterable<A>, predicate: Predicate.Predicate<A>): Effect.Effect<Array<A>>
  <A, E, R>(
    iterable: Iterable<A>,
    predicate: (a: NoInfer<A>, i: number) => Effect.Effect<boolean, E, R>,
    options?: { readonly concurrency?: Concurrency | undefined },
  ...
  <A, E, R>(
    elements: Iterable<A>,
    predicate: Predicate.Predicate<A> | ((a: A, i: number) => Effect.Effect<boolean, E, R>),
    options?: { readonly concurrency?: Concurrency | undefined },
  ): Effect.Effect<Array<A>, E, R> =>
    suspend(() => {
      const out: Array<A> = []
...
export const filterMap: {
  <A, B, X>(filter: Filter.Filter<NoInfer<A>, B, X>): (elements: Iterable<A>) => Effect.Effect<Array<B>>
  <A, B, X>(elements: Iterable<A>, filter: Filter.Filter<NoInfer<A>, B, X>): Effect.Effect<Array<B>>
} = dual(
  (args) => isIterable(args[0]) && !isEffect(args[0]),
  <A, B, X>(elements: Iterable<A>, filter: Filter.Filter<A, B, X>): Effect.Effect<Array<B>> =>
    suspend(() => {
      const out: Array<B> = []
  ...
  withFiber((fiber) => {
    ...
    return onExit(self, (_) => {
      ...
      if (children === undefined || children.size === 0) {
        ...
        children = Iterable.filter(children, (child: FiberImpl<any, any>) => !initialChildren.includes(child)) as Set<
          FiberImpl<any, any>
        >
      }
...
/** @internal */
export const runSyncExit: <A, E>(effect: Effect.Effect<A, E>) => Exit.Exit<A, E> = runSyncExitWith(Context.empty())
...
/** @internal */
export const isAsyncFiberError = (u: unknown): u is Cause.AsyncFiberError => hasProperty(u, AsyncFiberErrorTypeId)
...
/** @internal */
export const ConsoleRef = Context.Reference<Console.Console>('effect/Console/CurrentConsole', {
...
/** @internal */
export const CurrentLoggers = Context.Reference<ReadonlySet<Logger.Logger<unknown, any>>>(

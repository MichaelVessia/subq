 ...
 * @category References
 */
export const MetricRegistry = Context.Reference<Map<string, Metric.Metadata<any, any>>>(MetricRegistryKey, {
...
export const summary = (
  ...
  mapInput(
    ...
    (input, context) =>
      [input, Context.get(context, InternalEffect.ClockRef).currentTimeMillisUnsafe()] as [number, number],
  )
...
export const value = <Input, State>(self: Metric<Input, State>): Effect<State> =>
  InternalEffect.flatMap(InternalEffect.context(), (context) => InternalEffect.sync(() => self.valueUnsafe(context)))

...
export class Decoding<S extends Schema.Top> {
   ...
   * ```
   */
  async succeed<S extends Schema.Decoder<unknown, never>>(this: Decoding<S>, input: unknown): Promise<void>
  async succeed<S extends Schema.Decoder<unknown, never>>(
    this: Decoding<S>,
    input: unknown,
    expected: S['Type'],
  ): Promise<void>
  async succeed<S extends Schema.Decoder<unknown, never>>(this: Decoding<S>, input: unknown, expected?: S['Type']) {
    const r = await Effect.runPromise(
   ...
   * ```
   */
  async fail<S extends Schema.Decoder<unknown, never>>(this: Decoding<S>, input: unknown, message: string) {
    const r = await Effect.runPromise(
      this.decodeUnknownEffect(input, this.options?.parseOptions).pipe(
        Effect.mapError((issue) => issue.toString()),
        Effect.result,
...
class Encoding<S extends Schema.Top> {
   ...
   * ```
   */
  async succeed<S extends Schema.Encoder<unknown, never>>(this: Encoding<S>, input: unknown): Promise<void>
  async succeed<S extends Schema.Encoder<unknown, never>>(
    this: Encoding<S>,
    input: unknown,
    expected: S['Encoded'],
  ): Promise<void>
  async succeed<S extends Schema.Encoder<unknown, never>>(this: Encoding<S>, input: unknown, expected?: S['Encoded']) {
    const r = await Effect.runPromise(
   ...
   * ```
   */
  async fail<S extends Schema.Encoder<unknown, never>>(this: Encoding<S>, input: unknown, message: string) {
    const r = await Effect.runPromise(
      this.encodeUnknownEffect(input, this.options?.parseOptions).pipe(
        Effect.mapError((issue) => issue.toString()),
        Effect.result,

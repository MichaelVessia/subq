...
function resolveTailThreshold(inputLen: number, elementLen: number, tailLen: number) {
...
>()({
  onItem(
    ...
    const value: Option.Option<unknown> = Object.hasOwn(s.input, p.name) ? Option.some(s.input[p.name]) : Option.none()
  ...
  switch (ast._tag) {
    ...
    case 'Arrays':
      return ast.elements.flatMap((e, i) => {
        return isLiteral(e) && !isOptional(e) ? [{ key: i, literal: e.literal }] : []
      })
    case 'Suspend':
...
/** @internal */
export function withConstructorDefault<A extends AST>(ast: A, defaultValue: Effect.Effect<unknown>): A {
...
export const Json = new Declaration(
  [],
  () => (input, ast) =>
    isJson(input) ? Effect.succeed(input) : Effect.fail(new Issue.InvalidType(ast, Option.some(input))),
  {
    typeConstructor: {
      _tag: 'effect/Json',
    },
...
const StringTree = new Declaration(
  [],
  () => (input, ast) =>
    isStringTree(input) ? Effect.succeed(input) : Effect.fail(new Issue.InvalidType(ast, Option.some(input))),
  {
    expected: 'StringTree',
...
/** @internal */
export const unknownToStringTree = new Link(StringTree, Transformation.passthrough())

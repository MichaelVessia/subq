    ...
    fromStep(
      effect.sync(() => {
        ...
        > {
          ...
          return toStep<Result.Result<Output, Output2>, Input & Input2, Error | Error2, Env | Env2>(
            currentSide === 0 ? left : right,
          ).pipe(
            effect.flatMap((step) => {
              currentSide++
              if (currentSide === 1) {
            ...
            Pull.matchEffect(stepLeft(now, input as Input), {
              onSuccess: (leftResult) =>
                stepRight(now, input as Input2).pipe(
                  effect.map(
                    (rightResult) =>
                      [combine(leftResult[0], rightResult[0]), Duration.max(leftResult[1], rightResult[1])] as [
                        Output3,
                        Duration.Duration,
                      ],
                  ),
...
export const duration = (durationInput: Duration.Input): Schedule<Duration.Duration> => {
  ...
  return fromStepWithMetadata(
    effect.succeed((meta) => (meta.attempt === 1 ? effect.succeed([duration, duration]) : Cause.done(Duration.zero))),
  )
}
            ...
            Pull.matchEffect(stepLeft(now, input as Input), {
              ...
              onDone: (leftDone) =>
                stepRight(now, input as Input2).pipe(
                  effect.map(
                    (rightResult) =>
                      [combine(leftDone, rightResult[0]), rightResult[1]] as [Output3, Duration.Duration],
                  ),
                  Pull.catchDone((rightDone) => Cause.done(combine(leftDone, rightDone as Output2))),
                ),
            }),
 ...
 * @category constructors
 */
export const exponential = (base: Duration.Input, factor: number = 2): Schedule<Duration.Duration> => {
...
export const modifyDelay: {
  <Output, Error2 = never, Env2 = never>(
    f: (output: Output, delay: Duration.Duration) => Effect<Duration.Input, Error2, Env2>,
  ): <Input, Error, Env>(
    self: Schedule<Output, Input, Error, Env>,
  ) => Schedule<Output, Input, Error | Error2, Env | Env2>
  <Output, Input, Error, Env, Error2 = never, Env2 = never>(
    self: Schedule<Output, Input, Error, Env>,
    f: (output: Output, delay: Duration.Input) => Effect<Duration.Input, Error2, Env2>,
  ): Schedule<Output, Input, Error | Error2, Env | Env2>
} = dual(
  2,
  <Output, Input, Error, Env, Error2 = never, Env2 = never>(
      ...
      effect.map(
        ...
        (step) => (now, input) =>
          effect.flatMap(step(now, input), ([output, delay]) =>
            effect.map(f(output, delay), (delay) => [output, Duration.fromInputUnsafe(delay)]),
          ),
      ),
    ),
)
      ...
      effect.map(toStep(self), (step) => {
        ...
        return (now, input) =>
          effect.flatMap(step(now, input), (result) => {
            ...
            return effect.flatMap(isEffect(eff) ? eff : effect.succeed(eff), (check) =>
...
export const setInputType =
  <T>() =>
  <Output, Error, Env>(self: Schedule<Output, T, Error, Env>): Schedule<Output, T, Error, Env> =>
    self

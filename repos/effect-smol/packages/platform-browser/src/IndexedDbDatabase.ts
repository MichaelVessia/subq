...
const SchemaProto = {
  ...
  add<Version extends IndexedDbVersion.AnyWithProps>(
    ...
    version: Version,
    migrate: (fromQuery: Transaction<any>, toQuery: Transaction<Version>) => Effect.Effect<void, Error>,
  ) {
 ...
 * @category models
 */
export type IndexFromTableName<Version extends IndexedDbVersion.AnyWithProps, Table extends string> = IndexFromTable<
 ...
 */
export const make = <InitialVersion extends IndexedDbVersion.AnyWithProps, Error>(
...
const makeMigration = <
  ...
  Error,
>(options: {
  readonly previous: IndexedDbSchema<FromVersion, ToVersion, Error> | IndexedDbSchema<never, FromVersion, Error>
  readonly fromVersion: FromVersion
  readonly version: ToVersion
  readonly migrate: (fromQuery: Transaction<FromVersion>, toQuery: Transaction<ToVersion>) => Effect.Effect<void, Error>
...
const layer = <DatabaseName extends string>(databaseName: DatabaseName, migration: Any) =>
  Layer.effect(
    IndexedDbDatabase,
    Effect.gen(function* () {
...
const makeTransactionProto = <Source extends IndexedDbVersion.AnyWithProps>({
  ...
  migration.createIndex = Effect.fnUntraced(function* (table: string, indexName: string, options?: IDBIndexParameters) {
    const store = transaction.objectStore(table)
    const sourceTable = tables.get(table)!

    const keyPath = yield* Effect.fromNullishOr(sourceTable.indexes[indexName]).pipe(
      Effect.mapError(
        (cause) =>
          new IndexedDbDatabaseError({
            reason: 'MissingIndex',

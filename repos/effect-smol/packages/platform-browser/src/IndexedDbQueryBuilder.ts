...
export declare namespace IndexedDbQuery {
   ...
   * @category models
   */
  export type SelectType<Table extends IndexedDbTable.AnyWithProps> = [IndexedDbTable.KeyPath<Table>] extends [
    undefined,
  ]
   ...
   * @category models
   */
  export type ModifyType<Table extends IndexedDbTable.AnyWithProps> = (IndexedDbTable.AutoIncrement<Table> extends true
    ? {
  ...
  export interface First<
    Table extends IndexedDbTable.AnyWithProps,
    Index extends IndexedDbDatabase.IndexFromTable<Table>,
  > extends Effect.Effect<SelectType<Table>, IndexedDbQueryError, IndexedDbTable.Context<Table>> {
...
type IndexedDbValidNumberKeys<TableSchema extends IndexedDbTable.AnySchemaStruct> =
  keyof TableSchema['Encoded'] extends infer K
    ? K extends keyof TableSchema['Encoded']
...
const getReadonlyObjectStore = (query: IndexedDbQuery.Select<any, never> | IndexedDbQuery.Count<any, never>) => {
...
const applyFirst = Effect.fnUntraced(function* (query: IndexedDbQuery.First<any, never>) {
  ...
  return yield* Schema.decodeUnknownEffect(query.select.from.table.readSchema)(data).pipe(
    Effect.mapError(
      (error) =>
        new IndexedDbQueryError({
          reason: 'DecodeError',
...
const applyModify = Effect.fnUntraced(function* ({ query, value }: { query: IndexedDbQuery.Modify<any>; value: any }) {
  ...
  const encodedValue = yield* SchemaParser.makeEffect(
    autoIncrement && value[keyPath] === undefined ? table.autoincrementSchema : table.tableSchema,
  )(value).pipe(
...
const applyModifyAll = Effect.fnUntraced(
  function* ({ query, values }: { query: IndexedDbQuery.ModifyAll<any>; values: Array<any> }) {
    const autoIncrement = query.from.table.autoIncrement as boolean
    const keyPath = query.from.table.keyPath
    ...
    return yield* Effect.callback<Array<globalThis.IDBValidKey>, IndexedDbQueryError>((resume) => {
      const database = query.from.database
      const transaction = getOrCreateTransaction(database.current, [query.from.table.tableName], 'readwrite', {
        durability: query.from.table.durability,
      })
  ...
  Effect.catchIf(SchemaIssue.isIssue, (issue) =>
    Effect.fail(new IndexedDbQueryError({ reason: 'EncodeError', cause: new Schema.SchemaError(issue) })),
  ),
)
...
const applyClearAll = (options: { readonly database: globalThis.IDBDatabase }) =>
  Effect.callback<void, IndexedDbQueryError>((resume) => {
    const database = options.database
...
const FromProto: Omit<IndexedDbQuery.From<any>, 'table' | 'database' | 'IDBKeyRange' | 'transaction' | 'reactivity'> = {
  ...CommonProto,
  select<Index extends IndexedDbDatabase.IndexFromTable<any>>(this: IndexedDbQuery.From<any>, index?: Index) {
    return makeSelect({
      from: this,
      index,
    }) as any
  },
  count<Index extends IndexedDbDatabase.IndexFromTable<any>>(this: IndexedDbQuery.From<any>, index?: Index) {
    return makeCount({
      from: this,
      index,
    }) as any
  },
  delete<Index extends IndexedDbDatabase.IndexFromTable<any>>(this: IndexedDbQuery.From<any>, index?: Index) {
...
const DeletePartialProto: Omit<IndexedDbQuery.DeletePartial<any, never>, 'from' | 'index'> = {
  ...CommonProto,
...
> = {
  ...
  invalidate(
    ...
    keys ??=
      this.only !== undefined ? { [this.delete.from.table.tableName]: [this.only] } : [this.delete.from.table.tableName]
...
const CountProto: Omit<
  ...
  'from' | 'index' | 'only' | 'lowerBound' | 'upperBound' | 'excludeLowerBound' | 'excludeUpperBound'
> = {
  ...CommonProto,
...
const ModifyProto: Omit<IndexedDbQuery.Modify<any>, 'from' | 'value' | 'operation'> = {
  ...CommonProto,
...
const ModifyAllProto: Omit<IndexedDbQuery.ModifyAll<any>, 'from' | 'values' | 'operation'> = {
  ...CommonProto,
...
const QueryBuilderProto: Omit<
  ...
  'tables' | 'database' | 'IDBKeyRange' | 'IDBTransaction' | 'reactivity'
> = {
  ...CommonProto,

...
export const layerBackingIndexedDb = (options?: {
  ...
  Layer.effect(Persistence.BackingPersistence)(
    Effect.gen(function* () {
      const db = yield* Effect.acquireRelease(openDatabase(options?.database ?? defaultDatabase), (db) =>
...
export const layerIndexedDb = (options?: {
  readonly database?: string | undefined
}): Layer.Layer<Persistence.Persistence> => Persistence.layer.pipe(Layer.provide(layerBackingIndexedDb(options)))
...
const getMany = (
  ...
  withEntriesTransaction(db, 'readwrite', 'Failed to getMany from backing store', (entries, setResult, fail) => {
    const now = clock.currentTimeMillisUnsafe()
...
const set = (
  ...
  withEntriesTransaction(db, 'readwrite', `Failed to set key ${key} in backing store`, (entries, setResult, fail) => {
...
const setMany = (
  ...
  withEntriesTransaction(db, 'readwrite', 'Failed to setMany in backing store', (store, setResult, fail) => {
...
const remove = (db: IDBDatabase, storeId: string, key: string): Effect.Effect<void, Persistence.PersistenceError> =>
...
const withEntriesTransaction = <A>(
  ...
  message: string,
  run: (entries: IDBObjectStore, onResult: (result: A) => void, fail: (cause: unknown) => void) => void,

...
const provideDb = (database: IndexedDbDatabase.Any) =>
  Effect.provide(database.layer(databaseName).pipe(Layer.provide(layerFakeIndexedDb)))
...
class VerifyContext extends Context.Service<VerifyContext, { readonly maxLength: number }>()('VerifyContext') {}
...
describe.sequential('IndexedDbQueryBuilder', () => {
  ...
  describe('modify', () => {
    ...
    it.effect('insert with auto-increment', () => {
      ...
      return Effect.gen(function* () {
        const api = yield* Db.getQueryBuilder
        const addedKey = yield* api.from('price').insertAll([{ amount: 10 }, { amount: 20, id: 10 }, { amount: 30 }])
    ...
    it.effect('insert with auto-increment schema and different key path', () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.equal(data.length, 1)
        assert.deepStrictEqual(data, [new ProductSchema({ key: 10, name: 'insert1', price: 12 })])
      }).pipe(provideDb(Db))
    })
    ...
    it.effect('delete with limit', () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.equal(data.length, 1)
        assert.deepStrictEqual(data, [{ id: 12, title: 'insert3', count: 12, completed: true }])
      }).pipe(provideDb(Db))
    })
  ...
  describe('modify all', () => {
    ...
    it.effect('upsertAll same key', () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.equal(data.length, 1)
        assert.deepStrictEqual(data, [{ id: 10, title: 'update2', count: -11, completed: false }])
      }).pipe(provideDb(Db))
    })
  ...
  it.effect('stream', () => {
    class Db extends IndexedDbDatabase.make(
      ...
      Effect.fn(function* (api) {
        ...
        yield* api
          .from('todo')
          .insertAll(Array.makeBy(1000, (i) => ({ id: i + 1, title: `test${i}`, count: i, completed: false })))
      }),
    ) {}

    return Effect.gen(function* () {
      const api = yield* Db
      const data = yield* api.from('todo').select().stream().pipe(Stream.runCollect)
      assert.equal(data.length, 1000)

      const data2 = yield* api.from('todo').select().limit(10).stream().pipe(Stream.runCollect)
      assert.equal(data2.length, 10)
      assert.equal(data2[0].id, 1)

      const data3 = yield* api.from('todo').select().limit(10).offset(50).stream().pipe(Stream.runCollect)

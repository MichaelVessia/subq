...
describe.sequential('BrowserPersistence', () => {
  ...
  it.effect('custom database option', () =>
    Effect.gen(function* () {
      ...
      const fromDefault = yield* withStore('users', undefined, (store) => store.get('shared'))
      const fromCustom = yield* withStore('users', { database: customDatabase }, (store) => store.get('shared'))
...
const getRawEntry = (database: string, storeId: string, id: string): Effect.Effect<EntryRow | undefined, unknown> =>
  withDatabase(database, (db) =>
    idbRequest(() => db.transaction('entries', 'readonly').objectStore('entries').get([storeId, id])),
  )

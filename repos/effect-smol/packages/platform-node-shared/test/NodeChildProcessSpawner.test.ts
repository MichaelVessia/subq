  ...
  it.layer(NodeServices)((it) => {
    ...
    describe('error handling', () => {
      ...
      it.effect('should fail for invalid command', () =>
        Effect.gen(function* () {
          const exit = yield* Effect.exit(ChildProcess.make('nonexistent-command-12345'))
      ...
      it.effect('should handle spawn error with invalid cwd', () =>
        Effect.gen(function* () {
          const exit = yield* Effect.exit(ChildProcess.make('echo', ['test'], { cwd: '/nonexistent/directory/path' }))
    ...
    describe('process supervision', () => {
      const countMatchingProcesses = (pattern: string) =>
        Effect.gen(function* () {
          const handle = yield* ChildProcess.make('bash', ['-c', `ps aux | grep '${pattern}' | grep -v grep | wc -l`])
          const output = yield* decodeByteStream(handle.stdout)

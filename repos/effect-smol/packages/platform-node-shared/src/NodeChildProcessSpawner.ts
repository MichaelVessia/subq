...
const make = Effect.gen(function* () {
  ...
  const spawn = (command: ChildProcess.StandardCommand, spawnOptions: NodeChildProcess.SpawnOptions) =>
  ...
  const killProcessGroupOnExit = (childProcess: NodeChildProcess.ChildProcess, signal: NodeJS.Signals): void => {
    if (globalThis.process.platform === 'win32') {

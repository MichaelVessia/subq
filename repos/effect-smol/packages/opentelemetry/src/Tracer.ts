 ...
 * @category Services
 */
export class OtelTracer extends Context.Service<OtelTracer, Otel.Tracer>()('@effect/opentelemetry/Tracer') {}

/**
 * @since 1.0.0
 * @category Services
 */
export class OtelTracerProvider extends Context.Service<OtelTracerProvider, Otel.TracerProvider>()(
 ...
 * @category Services
 */
export class OtelTraceFlags extends Context.Service<OtelTraceFlags, Otel.TraceFlags>()(
 ...
 * @category Services
 */
export class OtelTraceState extends Context.Service<OtelTraceState, Otel.TraceState>()(
...
export const make: Effect.Effect<Tracer.Tracer, never, OtelTracer> = Effect.map(Effect.service(OtelTracer), (tracer) =>
  Tracer.make({
    ...
    context(primitive, fiber) {
      ...
      return Otel.context.with(populateContext(Otel.context.active(), currentSpan), () =>
...
const makeSpanContext = (span: Tracer.AnySpan, annotations?: Context.Context<never>): Otel.SpanContext => {
...
const makeTraceFlags = (span: Tracer.AnySpan, annotations: Context.Context<never> | undefined): Otel.TraceFlags => {

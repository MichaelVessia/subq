...
describe('Logger', () => {
  describe('provided', () => {
    ...
    it.effect('maps Effect LogLevel to OTel SeverityNumber spec values', () => {
      ...
      }).pipe(Effect.provide(SeverityLayer), Effect.provide(Layer.succeed(References.MinimumLogLevel, 'Trace')))
    })
    ...
    it.effect('uses monotonic clock timestamps and keeps them aligned with spans', () => {
      ...
      }).pipe(Effect.provide(TracingLive), Effect.provideService(Clock.Clock, skewedClock))
    })
  })
